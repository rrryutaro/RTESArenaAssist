...
def calc_layout_zones(form: 'LayoutForm', corner: 'LayoutCorner', dos_w: int, dos_h: int, lw: int, lh: int) -> list[tuple[int, int, int, int]]:
...
_RDW_INVALIDATE = 1
_RDW_UPDATENOW = 256
_RDW_ALLCHILDREN = 128
_GWL_STYLE = -16
_WS_CAPTION = 12582912
_WS_THICKFRAME = 262144
...
def _screen_of(widget) -> 'QRect':
    ...
    geo = sc.geometry()
    _log.debug('_screen_of: screen=%s geo=(%d,%d) %dx%d', sc.name(), geo.x(), geo.y(), geo.width(), geo.height())
    return geo
...
def _move_win(hwnd: int, x: int, y: int) -> bool:
    ret = ctypes.windll.user32.SetWindowPos(hwnd, None, x, y, 0, 0, _SWP_NOSIZE | _SWP_NOZORD | _SWP_NOACT)
    return bool(ret)

def _resize_win(hwnd: int, x: int, y: int, w: int, h: int) -> bool:
    ctypes.windll.kernel32.SetLastError(0)
    ret = ctypes.windll.user32.SetWindowPos(hwnd, None, x, y, w, h, _SWP_NOZORD | _SWP_NOACT)
    if not ret:
        err = ctypes.windll.kernel32.GetLastError()
        _log.warning('SetWindowPos failed: hwnd=%s pos=(%d,%d) size=%dx%d err=%d', hwnd, x, y, w, h, err)
    return bool(ret)
...
class LayoutManager(QObject):
        ...
        try:
            ...
            if self._dosbox_pid:
                ...
                if hwnds:
                    ...
                    _log.info('DOSBox (by PID) HWND=%s  title=%r  rect=%s', hwnds[0], title, rect)
            ...
            if hwnds:
                title = _get_title(hwnds[0])
                rect = _get_rect(hwnds[0])
                _log.info('DOSBox (by prefix) HWND=%s  title=%r  rect=%s', hwnds[0], title, rect)
                self._dosbox_hwnd = hwnds[0]
            else:
                _log.warning("DOSBox window not found (pid=%s, prefix='DOSBox')", self._dosbox_pid)
                self._dosbox_hwnd = 0
        except Exception:
    ...
    def _valid(self) -> bool:
        return bool(self._dosbox_hwnd and ctypes.windll.user32.IsWindow(self._dosbox_hwnd))
    ...
    def strip_dosbox_chrome(self) -> bool:
        ...
        ctypes.windll.user32.SetWindowLongW(hwnd, _GWL_STYLE, style & ~(_WS_CAPTION | _WS_THICKFRAME))
        ctypes.windll.user32.SetWindowPos(hwnd, None, 0, 0, 0, 0, _SWP_NOSIZE | _SWP_NOMOVE | _SWP_NOZORD | _SWP_NOACT | _SWP_FRAMECHANGED)
    ...
    def restore_dosbox_chrome(self) -> None:
        if self._layout_original_style is None:
            return
        if self._valid():
            ctypes.windll.user32.SetWindowLongW(self._dosbox_hwnd, _GWL_STYLE, self._layout_original_style)
            ctypes.windll.user32.SetWindowPos(self._dosbox_hwnd, None, 0, 0, 0, 0, _SWP_NOSIZE | _SWP_NOMOVE | _SWP_NOZORD | _SWP_NOACT | _SWP_FRAMECHANGED)
            _log.info('restore_dosbox_chrome: hwnd=%s  style=0x%08X', self._dosbox_hwnd, self._layout_original_style)
    ...
    def set_dosbox_topmost(self, enable: bool) -> None:
        ...
        ctypes.windll.user32.SetWindowPos(self._dosbox_hwnd, insert_after, 0, 0, 0, 0, _SWP_NOSIZE | _SWP_NOMOVE | _SWP_NOACT)
    ...
    def enter_embed_mode(self, parent_widget, x: int, y: int, w: int, h: int):
        ...
        ctypes.windll.user32.SetWindowLongW(hwnd, _GWL_STYLE, style & ~(_WS_CAPTION | _WS_THICKFRAME))
        ctypes.windll.user32.SetWindowPos(hwnd, None, 0, 0, 0, 0, _SWP_NOSIZE | _SWP_NOZORD | _SWP_NOACT | _SWP_FRAMECHANGED)
        ...
        def _repaint(h=hwnd, c=container):
            ctypes.windll.user32.RedrawWindow(h, None, None, _RDW_INVALIDATE | _RDW_UPDATENOW | _RDW_ALLCHILDREN)
            c.repaint()
    ...
    def exit_embed_mode(self) -> None:
        ...
        if self._valid() and self._embed_original_style is not None:
            ctypes.windll.user32.SetWindowLongW(self._dosbox_hwnd, _GWL_STYLE, self._embed_original_style)
            ctypes.windll.user32.SetWindowPos(self._dosbox_hwnd, None, 0, 0, 0, 0, _SWP_NOSIZE | _SWP_NOZORD | _SWP_NOACT | _SWP_FRAMECHANGED)
    ...
    def arrange(self, dos_w: int, dos_h: int, assist_win, corner: LayoutCorner=LayoutCorner.TOP_LEFT) -> bool:
        ...
        try:
            ...
            _log.info('SetWindowPos DOSBox: pos=(%d,%d) size=%dx%d', dos_x, dos_y, dos_w, dos_h)
