...
class I18nV2:
    ...
    def __init__(self, bundle: _bundle.Bundle, localpack: _localpack.Localpack | None=None, modset: _mods.ModSet | None=None):
        self.bundle = bundle
    ...
    @classmethod
    def load(cls, *, bundle_path: str | None=None, bundle_obj: dict | None=None, localpack_path: str | None=None, mods_dir: str | None=None) -> 'I18nV2':
    ...
    def _check_registry_compat(self) -> None:
        ...
        if self.localpack is not None and bhash and self.localpack.registry_hash:
            if self.localpack.registry_hash != bhash:
                self.warnings.append('registry_hash mismatch: localpack vs bundle (continuing, ids applied partially)')
        if self.mods is not None:
            known = set(self.bundle.all_ids())
            for mod in self.mods.mods:
                if bhash and mod.registry_hash and (mod.registry_hash != bhash):
                    self.warnings.append(f'registry_hash mismatch: mod {mod.filename} vs bundle (continuing)')
                unknown = [i for i in mod.texts if i not in known]
                if unknown:
                    self.warnings.append(f'mod {mod.filename} has {len(unknown)} id(s) not in registry (ignored)')
    ...
    def debug_resolve(self, id: int, locale: str) -> dict:
        return {'id': int(id), 'locale': locale, 'mod': self.mods.text(id, locale) if self.mods else None, 'bundle': self.bundle.locale_text(id, locale), 'original': self.resolve_original_surface(id), 'display': self.resolve_text(id, locale), 'source_policy': self.bundle.source_policy_of(id)}
__all__ = ['I18nV2']
