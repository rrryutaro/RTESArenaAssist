...
from PySide6.QtWidgets import QApplication, QCheckBox, QComboBox, QDialog, QDialogButtonBox, QFileDialog, QFontComboBox, QFormLayout, QGroupBox, QHBoxLayout, QLabel, QLineEdit, QMainWindow, QMenu, QMessageBox, QPushButton, QSpinBox, QStatusBar, QTabWidget, QVBoxLayout, QWidget
import struct
import time
import logging
...
class AssistWindow(QMainWindow):
    ...
    def __init__(self):
            ...
            def _hl_target(full, cur, prefetched, _w=self):
                for _attr in ('_tab_translate', '_layout_translate_panel', '_tab_log'):
                    ...
                    if _wid is not None and hasattr(_wid, 'highlight_reading'):
                        try:
                            _wid.highlight_reading(full, cur, prefetched)
                        except Exception:
                            pass
            self._reading_highlighter.register(_hl_target)
            self._tts.set_segment_observer(self._reading_highlighter.on_segment)
        except Exception:
            _log.exception('reading highlighter setup failed')
        ...
        self._layout_corner = LayoutCorner(settings.get('layout_corner', LayoutCorner.TOP_LEFT.value))
        try:
        ...
        from session import SessionManager, NpcChatSession, TavernSession, TempleSession, EquipmentSession, MagesGuildSession
    ...
    def _open_settings(self):
        ...
        if dlg.ui_language != settings.get('ui_language', ''):
            ...
            QMessageBox.information(self, i18n.tr('settings.language_restart_title'), i18n.tr('settings.language_restart_msg'))
        ...
        settings.set_val('map_show_recenter_lines', dlg.map_show_recenter_lines)
        settings.set_val('map_chunk_coord_font_size', dlg.map_chunk_coord_font_size)
        ...
        settings.set_val('tts_target_conversation', dlg.tts_target_conversation)
        ...
        try:
            ...
            pass
        self._apply_tts_settings()
        settings.set_val('translate_tab_emulate_panel_hidden', dlg.translate_tab_emulate_panel_hidden)
    ...
    def _on_tts_pause_clicked(self):
        self._tts.pause_speaking()

    def _on_tts_resume_clicked(self):
        self._tts.resume_speaking()

    def _on_tts_stop_clicked(self):
    ...
    def _transition_top_level(self, new_state: str, reason: str) -> None:
        if self._top_level_state == new_state:
            return
        _log.info('top_level: %s → %s (reason: %s)', self._top_level_state, new_state, reason)
    ...
    def _detect_top_level_at_connect(self) -> None:
        from controllers.connect_flow_controller import detect_top_level_at_connect
    ...
    def _poll(self):
        ...
        if _log.isEnabledFor(logging.DEBUG):
            breakdown = ' '.join((f'{name}={ms:.1f}ms' for name, ms in phases.items()))
            ckline = ' '.join((f'{name}@{cum:.1f}' for name, cum in checkpoints))
            _log.debug('poll timing: total=%.1fms%s%s', _elapsed_ms, f' [{breakdown}]' if breakdown else '', f' ck[{ckline}]' if ckline else '')

    def _update_translate_tab(self, entry: dict) -> None:
        from controllers.translation_update_controller import update_translate_tab
        ...
        if active:
            try:
                ...
                if entry is not None:
                    ...
                    self._ui_router.update_panel_translation(p_orig, p_basic)
    ...
    def _activate_choose_attributes_panel(self, *, priority: int=0) -> None:
        if self._chargen_class_list_active:
            self._set_class_list_panel_mode(False)
        try:
            self._ui_router.set_panel_mode('choose_attributes', priority=priority)
        except AttributeError:
            pass
    ...
    def _save_class_id_mapping(self, cls_en: str) -> None:
        if self._analyzer is None or not cls_en:
            return
        try:
            cls_id = self._analyzer.read_bytes(self._anchor + 535, 1)[0]
        except OSError:
            return
    ...
    def _push_translation(self, original: str, translated: str, panel_original: str | None=None, panel_translated: str | None=None, speech_role: str | None=None) -> None:
        from controllers.translation_update_controller import push_translation
        push_translation(self, original, translated, panel_original=panel_original, panel_translated=panel_translated, speech_role=speech_role)

    def _get_cap_dir(self) -> str:
        return settings.get('capture_dir', '') or settings.get('backup_dir', '') or os.path.join(_USER_DIR, 'captures')
    ...
    def _apply_tts_settings(self) -> None:
        ...
        try:
            ...
            reset_prewarm_state(self)
        except Exception:
            pass
        hl = getattr(self, '_reading_highlighter', None)
        if hl is not None:
            try:
                hl.on_segment(None, None)
            except Exception:
                pass
    ...
    def _capture(self):
        ...
        try:
            ...
            if self._is_layout_active:
                ...
                img = sc.capture_screen_region(r.left, r.top, r.right - r.left, r.bottom - r.top)
                if img:
                    path = os.path.join(out_dir, f'cap_{cap_no:03d}_layout.png')
                    img.save(path)
                    self._sb.showMessage(i18n.tr('capture.saved', no=cap_no, count=1), 4000)
            ...
            game_path, viewer_path = sc.save_screenshots(out_dir=out_dir, cap_no=cap_no, widget=self, game_pid=game_pid, game_prefix='DOSBox')
            saved = [p for p in (game_path, viewer_path) if p]
            if saved:
                self._sb.showMessage(i18n.tr('capture.saved', no=cap_no, count=len(saved)), 4000)
                self._tab_capture.set_cap_dir(out_dir)
            else:
                self._sb.showMessage(i18n.tr('capture.nothing'), 4000)
        except Exception as exc:
