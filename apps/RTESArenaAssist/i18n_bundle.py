...
def compute_registry_hash(raw: dict) -> str:
    ...
    payload = {'schema_version': int(raw.get('schema_version', 0)), 'registry_version': int(raw.get('registry_version', 0)), 'rows': rows}
    ...
    def _ingest_categories(self, cats: object) -> None:
        ...
        for cat in cats:
            ...
            if not name or not policy:
                raise BundleError(f'category missing name/source_policy: {cat!r}')
            entries = cat.get('entries') or []
            if not isinstance(entries, list):
                raise BundleError(f'category {name} entries must be a list')
            meta = {'category': name, 'source_policy': policy, 'source_provider': cat.get('source_provider'), 'id_range': cat.get('id_range'), 'entries': entries}
            self.categories[name] = meta
            for entry in entries:
                if not isinstance(entry, dict) or 'id' not in entry:
                    raise BundleError(f'entry missing id in category {name}: {entry!r}')
                eid = int(entry['id'])
...
def load_bundle_obj(raw: dict) -> Bundle:
    ...
    if ver > SUPPORTED_SCHEMA_VERSION:
        raise BundleError(f'bundle schema_version {ver} newer than supported {SUPPORTED_SCHEMA_VERSION}')
    return Bundle(raw)
