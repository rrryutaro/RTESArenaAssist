...
NEGOT_TEMPLATE_OFFSET = 37534
NEGOT_TEMPLATE_MAXLEN = 256
NEGOT_RENDERED_OFFSET = 39034
...
NEGOTIATION_PROFILES: dict[str, dict[str, tuple[str, ...]]] = {'NEGOTBUT.IMG': {'buttons_en': ('ACCEPT', 'COUNTER', 'REJECT'), 'buttons_ja': ('承諾', '対案', '拒否')}, 'YESNO.IMG': {'buttons_en': ('YES', 'NO', 'CANCEL'), 'buttons_ja': ('はい', 'いいえ', 'キャンセル')}}

def get_negotiation_profile(img_name: str) -> Optional[dict]:
    return NEGOTIATION_PROFILES.get(img_name)
_PLACEHOLDER_MAP = {'%i': '%nr', '%lu': '%a', '%u': '%a', '%d': '%a', '%mm': '%a'}
_C_PH_PATTERN = re.compile('%(?:lu|mm|s|i|u|d)')
_ARENA_PH_PATTERN = re.compile('%([a-z][a-z0-9]*)\\b')
...
def _template_to_prefix_regex(canonical: str) -> Optional[re.Pattern]:
    ...
    for m in _ARENA_PH_PATTERN.finditer(canonical):
        name = m.group(1)
        out_parts.append(_escape_literal_flexible_ws(canonical[last_end:m.start()]))
        if name in seen:
...
def extract_negotiation_body(template_raw: str, rendered: Optional[str]) -> Optional[str]:
...
def read_negotiation_message(analyzer: 'ArenaMemoryAnalyzer', anchor: int) -> Optional[str]:
    template_raw = _read_nul_terminated_ascii(analyzer, anchor, NEGOT_TEMPLATE_OFFSET, NEGOT_TEMPLATE_MAXLEN)
    if not template_raw:
        return None
    try:
        rendered_raw = analyzer.read_bytes(anchor + NEGOT_RENDERED_OFFSET, NEGOT_RENDERED_MAXLEN)
    except (OSError, AttributeError):
        return None
...
def read_negotiation_diagnostic(analyzer: 'ArenaMemoryAnalyzer', anchor: int) -> tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
    raw_template = _read_nul_terminated_ascii(analyzer, anchor, NEGOT_TEMPLATE_OFFSET, NEGOT_TEMPLATE_MAXLEN)
    canonical = _canonicalize_template(raw_template) if raw_template else None
    try:
        rendered_raw = analyzer.read_bytes(anchor + NEGOT_RENDERED_OFFSET, NEGOT_RENDERED_MAXLEN)
        rendered = _decode_chunks(rendered_raw) or None
    except (OSError, AttributeError):
        rendered = None
    matched = extract_negotiation_body(raw_template, rendered) if raw_template and rendered else None
    return (raw_template, canonical, rendered, matched)
__all__ = ['NEGOT_TEMPLATE_OFFSET', 'NEGOT_TEMPLATE_MAXLEN', 'NEGOT_RENDERED_OFFSET', 'NEGOT_RENDERED_MAXLEN', 'NEGOT_MESSAGE_OFFSET', 'NEGOT_MESSAGE_MAXLEN', 'BUTTON_LABELS_EN', 'BUTTON_LABELS_JA', 'NEGOTIATION_PROFILES', 'get_negotiation_profile', 'extract_negotiation_body', 'read_negotiation_message', 'read_negotiation_diagnostic']
