...
def save_screenshots(out_dir: str, cap_no: int, widget=None, game_pid: int=0, game_prefix: str='DOSBox', composite_hwnd: int=0):
    if not _PIL_OK:
        raise RuntimeError('Pillow が見つかりません。\n  pip install pillow  を実行してください。')
    ...
    if composite_hwnd and img_game and img_viewer:
        try:
            ...
            rel_y = dr.top - vr.top
            _cap_log.debug('composite: viewer(%d,%d) dosbox(%d,%d) rel=(%d,%d) viewer_img=%s dosbox_img=%s', vr.left, vr.top, dr.left, dr.top, rel_x, rel_y, img_viewer.size, img_game.size)
