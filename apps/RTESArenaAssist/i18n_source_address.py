...
def template_id(block, record_index, copy=None) -> str:
    ...
    return _SEP.join((KIND_TEMPLATE, _seg(block), _seg(int(copy)), _seg(int(record_index))))
...
def magic_item_id(item_idx, spell_kind, spell_idx) -> str:
    return _SEP.join((KIND_MAGIC_ITEM, _seg(int(item_idx)), _seg(str(spell_kind)), _seg(int(spell_idx))))
...
def citydata_location_id(province_index, location_id) -> str:
    return _SEP.join((KIND_CITYDATA, _seg(int(province_index)), _seg(int(location_id))))
...
def compare_manifests(golden_entries: dict[str, str], local_entries: dict[str, str]) -> dict:
    gset, lset = (set(golden_entries), set(local_entries))
    missing = sorted(gset - lset)
    extra = sorted(lset - gset)
    drift = sorted((sid for sid in gset & lset if not _hash_equal(golden_entries[sid], local_entries[sid])))
    return {'missing': missing, 'extra': extra, 'drift': drift, 'ok': not (missing or extra or drift), 'counts': {'golden': len(golden_entries), 'local': len(local_entries), 'missing': len(missing), 'extra': len(extra), 'drift': len(drift)}}
