...
class ArenaPack:
    ...
    def create(cls, path: str) -> 'ArenaPack':
        ...
        conn.execute('CREATE TABLE files (name TEXT PRIMARY KEY, sha TEXT NOT NULL, raw_size INTEGER NOT NULL, data BLOB NOT NULL)')
        conn.execute('INSERT INTO meta(k, v) VALUES (?, ?)', (META_FORMAT, PACK_FORMAT_VERSION))
        conn.commit()
        return cls(conn)
    ...
    def set_meta(self, key: str, value: str) -> None:
        self._conn.execute('INSERT INTO meta(k, v) VALUES (?, ?) ON CONFLICT(k) DO UPDATE SET v=excluded.v', (key, str(value)))
        self._conn.commit()
    ...
    def put(self, name: str, data: bytes) -> None:
        ...
        self._conn.execute('INSERT INTO files(name, sha, raw_size, data) VALUES (?, ?, ?, ?) ON CONFLICT(name) DO UPDATE SET sha=excluded.sha, raw_size=excluded.raw_size, data=excluded.data', (n, sha, len(data), comp))
        self._conn.commit()
    ...
    def get(self, name: str) -> bytes | None:
        n = (name or '').replace('\\', '/')
        row = self._conn.execute('SELECT sha, raw_size, data FROM files WHERE name=?', (n,)).fetchone()
        if row is None:
            return None
    ...
    def exists(self, name: str) -> bool:
        n = (name or '').replace('\\', '/')
        return self._conn.execute('SELECT 1 FROM files WHERE name=?', (n,)).fetchone() is not None

    def names(self) -> list[str]:
        return [r[0] for r in self._conn.execute('SELECT name FROM files ORDER BY name')]

    def verify(self) -> list[str]:
        bad: list[str] = []
        for name, sha, raw_size in self._conn.execute('SELECT name, sha, raw_size FROM files'):
            try:
                data = self.get(name)
            except (PackError, zlib.error):
                bad.append(name)
                continue
            if data is None or len(data) != raw_size or hashlib.sha256(data).hexdigest() != sha:
                bad.append(name)
        return bad
...
def build_pack(path: str, files: dict[str, bytes], meta: dict[str, str]) -> None:
    with ArenaPack.create(path) as pack:
        ...
        for name, data in files.items():
            pack.put(name, data)
__all__ = ['ArenaPack', 'PackError', 'build_pack', 'META_FORMAT', 'META_GENERATOR', 'META_FINGERPRINT', 'PACK_FORMAT_VERSION']
