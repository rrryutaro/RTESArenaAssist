...
class ArenaMemoryAnalyzer:
    ...
    def get_image_path(self) -> Optional[str]:
        ...
        if not pid:
            return None
        h = self._kernel32.OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, False, pid)
        if not h:
            return None
        try:
            buf = ctypes.create_unicode_buffer(32768)
            size = wintypes.DWORD(len(buf))
            ok = self._kernel32.QueryFullProcessImageNameW(h, 0, buf, ctypes.byref(size))
    ...
    def attach(self) -> bool:
        ...
        rw_flags = PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION
        handle = self._kernel32.OpenProcess(rw_flags, False, pid)
        if handle:
            self._can_write = True
        else:
            handle = self._kernel32.OpenProcess(PROCESS_VM_READ | PROCESS_QUERY_INFORMATION, False, pid)
    ...
    def read_bytes(self, address: int, size: int) -> bytes:
        ...
        ok = self._kernel32.ReadProcessMemory(self.handle, ctypes.c_void_p(address), buf, size, ctypes.byref(bytes_read))
        if not ok:
    ...
    def write_bytes(self, address: int, data: bytes) -> int:
        ...
        ok = self._kernel32.WriteProcessMemory(self.handle, ctypes.c_void_p(address), buf, len(data), ctypes.byref(n))
        if not ok:
            error = ctypes.get_last_error()
    ...
    def _enum_readable_regions(self, start: int, end: int) -> list[tuple[int, int]]:
        ...
        while addr < end:
            ret = self._kernel32.VirtualQueryEx(self.handle, ctypes.c_void_p(addr), ctypes.byref(mbi), ctypes.sizeof(mbi))
            if not ret:
                break
    ...
    def scan_bytes(self, pattern: bytes, start: int, end: int) -> list[ScanResult]:
        if not self.is_attached():
            raise RuntimeError('プロセスにアタッチされていません。')
        return [ScanResult(address=addr, raw_bytes=raw, display_value=raw.hex(' ').upper()) for addr, raw in self._scan_pattern(pattern, start, end)]
    ...
    def format_hexdump(data: bytes, base_address: int, width: int=16) -> list[dict]:
        ...
        for i in range(0, len(data), width):
            ...
            lines.append({'addr': addr, 'hex_bytes': hex_bytes, 'ascii_str': ascii_str, 'raw': chunk})
        return lines
