...
def _color_match(actual: tuple[int, int, int], expected: list[int], tolerance: int) -> bool:
    r, g, b = actual
    er, eg, eb = expected
    return abs(r - er) <= tolerance and abs(g - eg) <= tolerance and (abs(b - eb) <= tolerance)

def detect_dialog(img: 'Image.Image', mapper: 'ArenaCoordMapper', registry: 'ObsRegistry', purpose_filter: Optional[str]=None, threshold: float=0.75) -> DetectionResult:
    points = registry.all()
    if purpose_filter:
        points = [p for p in points if p.get('purpose') == purpose_filter]
    if not points:
        return DetectionResult(state=DialogState.UNKNOWN, hit_count=0, total_count=0, hit_ratio=0.0, detail='no observation points registered')
    ...
    for obs in points:
        ...
        if matched:
            ...
            missed_detail.append(f"{obs['name']}@({ax},{ay}): got {actual} exp {tuple(obs['expected_rgb'])} tol {tol}")
        checked += 1
    if checked == 0:
        return DetectionResult(state=DialogState.UNKNOWN, hit_count=0, total_count=0, hit_ratio=0.0, detail='all observation points out of bounds')
    ratio = hit / checked
    state = DialogState.OPEN if ratio >= threshold else DialogState.CLOSED
    detail = f'hit {hit}/{checked} ({ratio:.0%})' + (f" | misses: {'; '.join(missed_detail)}" if missed_detail else '')
    arena_rect = None
    client_rect = None
    ...
    return DetectionResult(state=state, hit_count=hit, total_count=checked, hit_ratio=ratio, detail=detail, arena_rect=arena_rect, client_rect=client_rect)
