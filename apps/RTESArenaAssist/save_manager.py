...
_SLOT_EXT_RE = re.compile('\\.0[0-9]$', re.IGNORECASE)
_SHARED_FILES = frozenset({'NAMES.DAT'})
_EXT_DATA_SUBDIR = 'ext_data'
_EXT_SLOT_FILE_RE = re.compile('^.+\\.0([0-9])$', re.IGNORECASE)

def _ext_data_dir() -> str:
    from services.map_ext_store import ext_data_dir
    return ext_data_dir()

def _ext_slot_from_filename(filename: str) -> int | None:
    m = _EXT_SLOT_FILE_RE.match(filename)
    if not m:
        return None
    return int(m.group(1))

def _iter_ext_slot_files(ext_dir: str, slot: int) -> list[str]:
    if not os.path.isdir(ext_dir):
        return []
    result = []
    for fname in os.listdir(ext_dir):
        path = os.path.join(ext_dir, fname)
        if os.path.isfile(path) and _ext_slot_from_filename(fname) == slot:
            result.append(fname)
    return sorted(result)

def _remap_ext_slot_filename(filename: str, target_slot: int) -> str:
    return filename[:-1] + str(target_slot)
...
def _backup_ext_data(backup_path: str, slots: list[int]) -> None:
    ...
    for s in slots:
        for fn in _iter_ext_slot_files(src_dir, s):
...
def _restore_ext_data(backup_path: str, slots: list[int]) -> None:
    ...
    for s in slots:
        src_files = set(_iter_ext_slot_files(src_dir, s))
        for fn in _iter_ext_slot_files(dest_dir, s):
...
def _restore_ext_data_to_slot(backup_path: str, source_slot: int, target_slot: int) -> None:
    src_dir = os.path.join(backup_path, _EXT_DATA_SUBDIR)
    dest_dir = _ext_data_dir()
    remapped = {_remap_ext_slot_filename(fn, target_slot): fn for fn in _iter_ext_slot_files(src_dir, source_slot)}
    for fn in _iter_ext_slot_files(dest_dir, target_slot):
        if fn not in remapped:
            try:
                os.remove(os.path.join(dest_dir, fn))
            except OSError:
                pass
    for dst_fn, src_fn in sorted(remapped.items()):
        os.makedirs(dest_dir, exist_ok=True)
        shutil.copy2(os.path.join(src_dir, src_fn), os.path.join(dest_dir, dst_fn))
    ...
    for f in os.listdir(game_dir):
        ...
        if m:
            ...
            if slots is None or slot in slots:
                targets.append(f)
    if not targets:
        raise FileNotFoundError(f'セーブファイル (*.00〜*.09) が見つかりません: {game_dir}')
...
def create_backup(game_dir: str, backup_dir: str, name: str='', tags=None, memo: str='', slots: list[int] | None=None) -> dict:
    if tags is None:
        tags = []
    ...
    for fname in save_files:
        shutil.copy2(os.path.join(game_dir, fname), os.path.join(backup_path, fname))
        copied.append(fname)
    slot_set = set()
    for fname in copied:
    ...
    for s in sorted(slot_set):
        ...
        if note.get('name') or note.get('memo'):
            slot_notes_capture[str(s)] = {'name': note.get('name', ''), 'memo': note.get('memo', '')}
    meta = {'id': folder_name, 'datetime': now.isoformat(timespec='seconds'), 'name': display_name, 'tags': tags, 'memo': memo, 'slots': sorted(slot_set), 'slot_names': slot_names, 'slot_notes': slot_notes_capture, 'files': copied}
    with open(os.path.join(backup_path, _META_FILE), 'w', encoding='utf-8') as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)
    _backup_ext_data(backup_path, sorted(slot_set))
    return meta

def restore_backup(game_dir: str, backup_dir: str, backup_id: str, slots: list[int] | None=None) -> None:
    backup_path = os.path.join(backup_dir, backup_id)
    if not os.path.isdir(backup_path):
        raise FileNotFoundError(f'バックアップが見つかりません: {backup_id}')
    all_backup_files = [f for f in os.listdir(backup_path) if f != _META_FILE and f != _EXT_DATA_SUBDIR]
    if slots is not None:
        restore_files = [f for f in all_backup_files if _SLOT_EXT_RE.search(f) and int(f[-1]) in slots]
    ...
    if os.path.isdir(game_dir):
        for f in os.listdir(game_dir):
            ...
            elif restore_shared and upper in _SHARED_FILES and any((fi.upper() in _SHARED_FILES for fi in restore_files)):
                os.remove(os.path.join(game_dir, f))
    os.makedirs(game_dir, exist_ok=True)
    for fname in restore_files:
        shutil.copy2(os.path.join(backup_path, fname), os.path.join(game_dir, fname))
    try:
...
def restore_backup_to_slot(game_dir: str, backup_dir: str, backup_id: str, source_slot: int, target_slot: int) -> None:
    backup_path = os.path.join(backup_dir, backup_id)
    if not os.path.isdir(backup_path):
        raise FileNotFoundError(f'バックアップが見つかりません: {backup_id}')
    src_ext_re = re.compile(f'\\.0{source_slot}$', re.IGNORECASE)
    source_files = [f for f in os.listdir(backup_path) if f != _META_FILE and src_ext_re.search(f)]
    if not source_files:
        raise FileNotFoundError(f'バックアップ内にスロット {source_slot} のファイルが見つかりません: {backup_id}')
    if os.path.isdir(game_dir):
        tgt_ext_re = re.compile(f'\\.0{target_slot}$', re.IGNORECASE)
        for f in os.listdir(game_dir):
    ...
    for fname in source_files:
        ...
        shutil.copy2(os.path.join(backup_path, fname), os.path.join(game_dir, new_fname))
    try:
        with open(os.path.join(backup_path, _META_FILE), encoding='utf-8') as f:
            backup_meta = json.load(f)
...
def update_meta(backup_dir: str, backup_id: str, name: str, tags: list, memo: str) -> dict:
    backup_path = os.path.join(backup_dir, backup_id)
...
def update_backup_slot_note(backup_dir: str, backup_id: str, slot: int, name: str, memo: str) -> dict:
    backup_path = os.path.join(backup_dir, backup_id)
    meta_path = os.path.join(backup_path, _META_FILE)
    with open(meta_path, encoding='utf-8') as f:
        meta = json.load(f)
