...
def _resolve_data_dir() -> str:
    ...
    if getattr(sys, 'frozen', False):
        candidates.append(os.path.join(getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(sys.executable))), 'services', 'data'))
...
def load_city_generation_data(path: Optional[str]=None) -> CityGenerationData:
    ...
    if path is not None:
        ...
        if raw is None:
            raise FileNotFoundError('city_generation data unavailable (pack/bundled both absent)')
    d = raw['data']
    _cached = CityGenerationData(coastal_city_list=list(d['coastal_city_list']), city_template_filenames=list(d['city_template_filenames']), starting_positions=[tuple(p) for p in d['starting_positions']], reserved_block_lists=[list(arr) for arr in d['reserved_block_lists']])
    return _cached
...
class LocationData:
    ...
    def city_seed(self) -> int:
        return (self.x & 65535) << 16 | self.y & 65535
...
def load_world_map_data(path: Optional[str]=None) -> WorldMapData:
    ...
    if path is not None:
        ...
        if raw is None:
            raise FileNotFoundError('world_map data unavailable (pack/bundled both absent)')

    def _loc(d: dict) -> LocationData:
        return LocationData(name=d['name'], x=d['x'], y=d['y'], visibility=d['visibility'])
    provinces = []
    for p in raw['provinces']:
        provinces.append(ProvinceData(name=p['name'], global_x=p['globalX'], global_y=p['globalY'], global_w=p['globalW'], global_h=p['globalH'], city_states=[_loc(c) for c in p['cityStates']], towns=[_loc(t) for t in p['towns']], villages=[_loc(v) for v in p['villages']], second_dungeon=_loc(p['secondDungeon']), first_dungeon=_loc(p['firstDungeon']), random_dungeons=[_loc(d) for d in p['randomDungeons']]))
