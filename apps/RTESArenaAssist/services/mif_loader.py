...
KNOWN_CHUNKS = {'FLAT', 'FLOR', 'INFO', 'INNS', 'LEVL', 'LOCK', 'LOOT', 'MAP1', 'MAP2', 'MHDR', 'NAME', 'NUMF', 'STOR', 'TARG', 'TRIG'}
...
def _parse_level(data: bytes, mif_path: Path, width: int, height: int, level_index: int) -> MifMap:
    ...
    return MifMap(path=mif_path, flor=flor, map1=map1, trigs=trigs, targs=targs, locks=locks, entities=entities, chunks=chunks, width=width, height=height, level_index=level_index, raw_map1_size=raw_map1_size, mif_name=mif_name_val, info_name=info_name_val)
...
def _parse_mif_scan_fallback(data: bytes, mif_path: Path) -> MifMap:
    ...
    return MifMap(path=mif_path, map1=list(map1_raw or b''), trigs=trigs, targs=targs, locks=locks, chunks=chunks, width=width, height=height, raw_map1_size=len(map1_raw or b''))

def _parse_trigs(body: bytes, start_order: int) -> list[TriggerRecord]:
    return [TriggerRecord(*struct.unpack_from('4B', body, n * 4), start_order + n) for n in range(len(body) // 4)]

def _parse_targs(body: bytes, start_order: int) -> list[TargetRecord]:
    return [TargetRecord(*struct.unpack_from('2B', body, n * 2), start_order + n) for n in range(len(body) // 2)]

def _parse_locks(body: bytes, start_order: int) -> list[LockRecord]:
    return [LockRecord(*struct.unpack_from('3B', body, n * 3), start_order + n) for n in range(len(body) // 3)]
...
def _decode_type08(src: bytes, out_size: int) -> bytes:
    high_offset_bits = [0] * 32
    for value in range(1, 4):
        high_offset_bits.extend([value] * 16)
    for value in range(4, 12):
        high_offset_bits.extend([value] * 8)
    for value in range(12, 24):
        high_offset_bits.extend([value] * 4)
    for value in range(24, 48):
        high_offset_bits.extend([value] * 2)
    high_offset_bits.extend(range(48, 64))
    low_offset_bit_count = [*(3 for _ in range(32)), *(4 for _ in range(48)), *(5 for _ in range(64)), *(6 for _ in range(48)), *(7 for _ in range(48)), *(8 for _ in range(16))]
    history = [32] * 4096
    historypos = 0
    node_idx_map = [0] * 941
    for i in range(626):
    ...
    def ensure_bits() -> tuple[int, int, int]:
        ...
        while validbits < 9:
            if src_pos < len(src):
                bitmask = (bitmask | src[src_pos] << 8 - validbits) & 65535
                src_pos += 1
            validbits += 8
    ...
    while len(out) < out_size:
        ...
        if codeword < 256:
            ...
            for _ in range(bitcount):
                ...
                bitmask = bitmask << 1 & 65535
                validbits -= 1
            copypos = historypos - (offset_high | offset_low & 63) - 1
            tocopy = codeword - 256 + 3
            for _ in range(tocopy):
                value = history[copypos & 4095]
                copypos += 1
                history[historypos & 4095] = value
                historypos += 1
                out.append(value)
...
def detect_dimensions(map1: bytes | None, trigs: list[TriggerRecord], targs: list[TargetRecord], locks: list[LockRecord]) -> tuple[int, int]:
...
def _extract_entities(map1: list[int], flor: list[int], width: int, height: int) -> list[EntityRecord]:
    ...
    for sn in range(height):
        for we in range(width):
            ...
            if (m1v & 61440) >> 12 == 8:
                entities.append(EntityRecord(x=we, y=sn, flat_index=m1v & 255, source='map1'))
            floor_flat_id = flv & 255
            if floor_flat_id > 0:
    ...
    for line in lines:
        ...
        if s.startswith('*'):
            if up.startswith('*MENU'):
                pending_menu = True
            elif up.startswith(('*TRANS', '*DOOR', '*BOXCAP', '*BOXSIDE', '*LAVACHASM', '*WETCHASM', '*DRYCHASM', '*WALKTHRU', '*TRANSWALKTHRU', '*LEVELUP', '*LEVELDOWN')):
                pass
            continue
        parts = s.split('#')
...
def parse_inf_level_transitions(path: str | Path) -> tuple[int | None, int | None]:
    ...
    for line in lines:
        ...
        if s.startswith('*'):
            ...
            elif up.startswith(('*TRANS', '*DOOR', '*BOXCAP', '*BOXSIDE', '*LAVACHASM', '*WETCHASM', '*DRYCHASM', '*WALKTHRU', '*TRANSWALKTHRU')):
                pass
            continue
        parts = s.split('#')
        set_count = int(parts[1].strip()) if len(parts) == 2 else 1
