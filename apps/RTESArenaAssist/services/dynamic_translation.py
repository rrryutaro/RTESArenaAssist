...
_AEXE_STRINGS_PATH = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'aexe_strings.json'))
_ASSIST_DIR = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))
...
def translate_tavern(t: TavernName) -> BuildingTranslation:
    ...
    if not ja:
        ...
        suffixes = (data.get('marine_suffixes') if t.coastal else data.get('suffixes')) or []
...
def translate_temple(t: TempleName) -> BuildingTranslation:
    ...
    if not 0 <= t.model < len(models):
        return BuildingTranslation(en='', ja=None, parts_missing=[f'temple.model[{t.model}]'])
    ...
    if not ja:
        ...
        _suf, suf_ja = _lookup_array_part(suffixes, t.suffix_index)
        missing.append(suf_en)
        _missing_parts.add(f'temple.models[{t.model}].suffixes: {suf_en}')
        if suf_ja:
