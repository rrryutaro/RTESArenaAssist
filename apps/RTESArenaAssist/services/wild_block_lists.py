...
class WildBlockLists:
    ...
    tavern: tuple[int, ...]
    temple: tuple[int, ...]
_FALLBACK_LISTS = WildBlockLists(normal=tuple(), village=tuple(), dungeon=tuple(), tavern=tuple(), temple=tuple())
_CACHED: Optional[WildBlockLists] = None
_CACHE_SOURCE: Optional[str] = None

def _is_plausible_list(data: bytes, offset: int, expected_size: int) -> bool:
...
def search_block_lists_in_buffer(buf: bytes) -> Optional[WildBlockLists]:
    ...
    if n < 64:
        return None
    end = n - (1 + NORMAL_SIZE + 1 + VILLAGE_SIZE + 1 + DUNGEON_SIZE + 1 + TAVERN_SIZE + 1 + TEMPLE_MIN)
    for off in range(end):
        ...
        _log.info('wild_block_lists: signature match at +0x%X (normal=%s village=%s dungeon=%s tavern=%s temple=%s)', off, list(normal), list(village), list(dungeon), list(tavern), list(temple))
        return WildBlockLists(normal=normal, village=village, dungeon=dungeon, tavern=tavern, temple=temple)
    return None

def load_block_lists_from_memory(analyzer, scan_start: int=0, scan_end: int=2147483647) -> Optional[WildBlockLists]:
    if analyzer is None:
        return None
    ...
    for base, size in regions:
        ...
        if result is not None:
            _log.info('wild_block_lists: signature match in region base=0x%08X size=0x%X', base, size)
            return result
...
def get_block_lists(analyzer=None) -> WildBlockLists:
    ...
    _log.warning('wild_block_lists: using empty fallback (= memory search failed). wilderness map outside city center will be mostly empty until block lists are obtained.')
    return _CACHED
...
def invalidate_cache() -> None:
    ...
    _CACHED = None
    _CACHE_SOURCE = None
__all__ = ['WildBlockLists', 'search_block_lists_in_buffer', 'load_block_lists_from_memory', 'get_block_lists', 'get_cache_source', 'invalidate_cache']
