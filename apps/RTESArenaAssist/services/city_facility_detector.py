...
_SERVICE_MARKERS: dict[ArenaMenuType, int] = {ArenaMenuType.EQUIPMENT: 1542, ArenaMenuType.TAVERN: 2827, ArenaMenuType.TEMPLE: 6168, ArenaMenuType.MAGES_GUILD: 4369}
_MENU_IDS: dict[ArenaMenuType, int] = {ArenaMenuType.EQUIPMENT: 0, ArenaMenuType.TAVERN: 1, ArenaMenuType.MAGES_GUILD: 2, ArenaMenuType.TEMPLE: 3}
    ...
    for entry in entries:
        ...
        for local_y, row in enumerate(level.map1):
            for local_x, voxel in enumerate(row):
                ...
                hits.append(_MarkerHit(original_x=original_x, original_y=original_y, block_type=entry.block_type, block_mif=entry.block_mif, local_x=local_x, local_y=local_y, marker_voxel=marker_voxel))
    return sorted(hits, key=lambda hit: (hit.original_y, hit.original_x))

def _make_mif_name(hit: _MarkerHit, menu_type: ArenaMenuType, city_type: ArenaCityType) -> str | None:
    return get_door_voxel_mif_name(x=hit.original_x, y=hit.original_y, menu_id=_MENU_IDS[menu_type], ruler_seed=0, palace_is_main_quest_dungeon=False, city_type=city_type, map_type=MapType.CITY)

def _equipment_with_names(city_seed: int, hits: list[_MarkerHit], city_type_key: str | None, city_type: ArenaCityType, race_id: int) -> list[FacilityPlacement]:
