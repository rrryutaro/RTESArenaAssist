...
def get_local_city_and_province_id(global_city_id: int) -> tuple[int, int]:
    return (global_city_id & 0x1F, global_city_id >> 5)
...
def get_sky_seed(local_point: Int2, province_id: int, province_rect: Rect) -> int:
    global_point = get_global_point(local_point, province_rect)
    seed = ((global_point.x & 0xFFFF) << 16) | (global_point.y & 0xFFFF)
    return (seed * province_id) & 0xFFFFFFFF
...
def get_city_template_name_index(location_type: ArenaLocationType,
                                 is_coastal: bool) -> int:
...
def get_city_starting_position_index(location_type: ArenaLocationType,
                                     is_coastal: bool,
                                     template_id: int) -> int:
    if location_type == ArenaLocationType.CITY_STATE:
