...
from PySide6.QtWidgets import QAbstractItemView, QButtonGroup, QComboBox, QFrame, QHBoxLayout, QHeaderView, QLabel, QLineEdit, QListWidget, QListWidgetItem, QPushButton, QSplitter, QStackedWidget, QTableWidget, QTableWidgetItem, QTextEdit, QVBoxLayout, QWidget
...
def _category_dict_rows(cat_id: str, cat_display: str) -> list:
    ...
    if src:
        for id_, e in src.items():
            ...
            if not eng:
                continue
            rows.append({'eng': eng, 'jpn': i18n.lang_only(id_) or '', 'cat_id': cat_id, 'cat_display': cat_display})
        return rows
    for e in i18n.v2_category_entries(cat_id):
        eng = e.get('original') or ''
        if not eng:
            continue
        rows.append({'eng': eng, 'jpn': e.get('text') or '', 'cat_id': cat_id, 'cat_display': cat_display})
    return rows
...
def _read_strings(path: str) -> dict[str, str]:
    ...
    if isinstance(data.get('strings'), dict):
        return {k: v for k, v in data['strings'].items() if isinstance(v, str)}
...
def _load_assist_ui_entries(lang: str) -> tuple[list, dict | None]:
    ...
    for key, en_val in en_strings.items():
        ...
        if not isinstance(cur_val, str):
            cur_val = ''
        entries.append({'eng': en_val, 'jpn': cur_val, 'cat_id': cat_id, 'cat_display': cat_display})
    if not entries:
        return ([], None)
...
class TabDict(QWidget):
    ...
    def _build_dict_page(self) -> QWidget:
        ...
        self._dict_table.setHorizontalHeaderLabels([i18n.tr('dict.col_eng'), i18n.tr('dict.col_jpn'), i18n.tr('dict.col_category')])
    ...
    def _build_inf_page(self) -> QWidget:
        ...
        for key, label in [('all', i18n.tr('inf_text.type_all')), ('lore', 'lore'), ('lore_once', 'lore_once'), ('riddle', 'riddle'), ('key', 'key'), ('key_lore', 'key_lore')]:
        ...
        self._inf_orig.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse | Qt.TextInteractionFlag.TextSelectableByKeyboard)
        sep = QFrame()
        ...
        self._inf_trans.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse | Qt.TextInteractionFlag.TextSelectableByKeyboard)
    ...
    def _search_dict(self) -> None:
        ...
        if keyword:
            filtered = [e for e in filtered if keyword in e['eng'].lower() or keyword in e['jpn'].lower()]
    ...
    def _show_dict_entries(self, entries: list) -> None:
        ...
        if entries:
            self._dict_result_lbl.setText(i18n.tr('dict.results', count=len(entries)))
        else:
            kw = self._dict_search_edit.text().strip()
            self._dict_result_lbl.setText(i18n.tr('dict.no_results') if kw else i18n.tr('dict.results', count=0))
    ...
    def _on_inf_changed(self) -> None:
        ...
        if inf_name:
            ...
            itl._ensure_loaded()
            self._inf_entries = sorted(itl._index.values(), key=lambda e: (e['inf'], e['idx']))
    ...
    def _on_inf_filter_changed(self) -> None:
        ...
        for e in self._inf_entries:
            ...
            if query and query not in text_body.lower() and (query not in trans_body.lower()):
                continue
            label = f"[{e['idx']:3d}] {e['type']:10s}  {text_body[:40].replace(chr(10), ' ')}"
            item = QListWidgetItem(label)
            item.setData(Qt.ItemDataRole.UserRole, e)
            self._inf_list.addItem(item)
        self._inf_list.blockSignals(False)

    def _on_inf_item_selected(self, cur: QListWidgetItem | None, _prev: QListWidgetItem | None) -> None:
        if cur is None:
            return
    ...
    def _show_inf_entry(self, entry: dict) -> None:
        t = entry.get('type', '')
        if t == 'riddle':
            orig_text = f"[問] {entry.get('question', '')}\n[正] {entry.get('correct', '')}\n[誤] {entry.get('wrong', '')}"
            trans = itl.get_translation(entry)
            if isinstance(trans, dict):
                trans_text = f"[問] {trans.get('question', '')}\n[正] {trans.get('correct', '')}\n[誤] {trans.get('wrong', '')}"
            else:
                trans_text = ''
        else:
        ...
        self._inf_trans.setStyleSheet(_TRANSLATED_STYLE if trans_text else _UNTRANSLATED_STYLE)
