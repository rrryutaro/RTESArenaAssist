...
class TabMap(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None,
        ...
        self._place_label.setStyleSheet(
            "QLabel#mapPlaceLabel {"
            "  padding: 4px 8px;"
            "  color: #c9d1e0;"
            "  font-weight: bold;"
            "  background: #1a2635;"
    ...
    def update_map_state(
        ...
        ctx = MapContext(
            ...
            save_dir=_save_dir,
            wall_los_enabled=self._wall_los_enabled,
            reveal_all=bool(settings.get("cheat_enabled", False))
                        and bool(settings.get("cheat_reveal_map", False)),
            show_unexplored_floor=bool(
                settings.get("map_show_unexplored_floor", False)),
            center_on_player=bool(
                settings.get("map_center_on_player", True)),
            ...
            wild_show_edge=(
                bool(settings.get("map_extended_display", True))
                and bool(settings.get("wild_show_edge", True))),
            wild_distinguish_edge=(
                bool(settings.get("map_extended_display", True))
                and bool(settings.get("wild_distinguish_edge", True))),
            wild_show_crops=(
                bool(settings.get("map_extended_display", True))
                and bool(settings.get("wild_show_crops", True))),
            wild_show_all_entrances=(
                bool(settings.get("map_extended_display", True))
                and bool(settings.get("wild_show_all_entrances", True))),
            wild_show_static_flats=(
                bool(settings.get("map_extended_display", True))
                and bool(settings.get("wild_show_static_flats", True))),
        )
        try:
        ...
        if getattr(self, "_diag_prev", None) != _diag:
            ...
            _log.warning(
                "tabmap[%s]: active=%s walkable=%s tab_visible=%s "
                "canvas_visible=%s canvas_size=%s",
