...
from PySide6.QtWidgets import QFormLayout, QHBoxLayout, QInputDialog, QLabel, QLineEdit, QListWidgetItem, QMessageBox, QPlainTextEdit, QPushButton, QSizePolicy, QTabWidget, QTableWidgetItem, QVBoxLayout, QWidget
...
class TabSave(QWidget):
    ...
    def _rebuild_detail_game_slot(self, slot: int):
        ...
        form.addRow(i18n.tr('save.slot_save_name') + ':', QLabel(info.get('save_name') or i18n.tr('save.parse_unknown')))
        form.addRow(i18n.tr('save.slot_modified') + ':', QLabel(info.get('modified') or '—'))
        form.addRow(i18n.tr('save.slot_files') + ':', QLabel(str(info.get('file_count', 0))))
        lay.addLayout(form)
    ...
    def _rebuild_detail_backup_slot(self, slot: int):
        ...
        if not meta:
            ...
            return
        self._clear_edit_refs()
        self._swap_detail_widget(self._build_backup_tabbed_widget(meta, slot=slot, initial_tab=1))

    def _build_backup_tabbed_widget(self, meta: dict, slot: int | None, initial_tab: int=0) -> QWidget:
        container = QWidget()
    ...
    def _get_game_dir(self) -> str | None:
        game_dir = cfg.get('save_dir', '').strip()
        if not game_dir:
            QMessageBox.warning(self, i18n.tr('common.warning'), i18n.tr('save.error_no_dir'))
            return None
        if not os.path.isdir(game_dir):
            QMessageBox.warning(self, i18n.tr('common.warning'), i18n.tr('save.error_dir_not_found').format(path=game_dir))
            return None
        return game_dir
    ...
    def _do_backup_checked(self):
        ...
        if not slots:
            QMessageBox.information(self, i18n.tr('common.warning'), i18n.tr('save.error_no_selection'))
            return
    ...
    def _run_create_backup(self, game_dir: str, slots: list[int] | None):
        name, ok = QInputDialog.getText(self, i18n.tr('save.create_backup'), i18n.tr('save.edit_name') + ':')
        if not ok:
            return
        backup_dir = _effective_backup_dir()
        try:
            meta = save_manager.create_backup(game_dir, backup_dir, name=name.strip(), slots=slots)
            self.status_message.emit(i18n.tr('save.status_created').format(name=meta['name']))
    ...
    def _do_restore_selected(self):
        if not self._current_backup_meta:
            return
        if self._current_slot is None:
            QMessageBox.information(self, i18n.tr('common.warning'), i18n.tr('save.no_selection'))
            return
        ...
        chosen, ok = QInputDialog.getItem(self, i18n.tr('save.restore_target_title'), i18n.tr('save.restore_target_prompt').format(source=source_slot), items, current=source_slot, editable=False)
        if not ok:
            return
        target_slot = items.index(chosen)
        if target_slot in occupied_slots:
            reply = QMessageBox.question(self, i18n.tr('common.confirm'), i18n.tr('save.restore_target_overwrite').format(target=target_slot), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            if reply != QMessageBox.StandardButton.Yes:
                return
        meta = self._current_backup_meta
        backup_dir = _effective_backup_dir()
        try:
            save_manager.restore_backup_to_slot(game_dir, backup_dir, meta['id'], source_slot, target_slot)
            self.status_message.emit(i18n.tr('save.status_restored').format(name=meta.get('name', '')))
            self._check_save_name_changes()
            self._refresh()
        except Exception as exc:
            QMessageBox.critical(self, i18n.tr('common.error'), str(exc))
    ...
    def _run_restore(self, slots: list[int] | None):
        ...
        if not game_dir:
            return
        backup_dir = _effective_backup_dir()
        slots_str = ', '.join((str(s) for s in (slots if slots is not None else meta.get('slots', []))))
        reply = QMessageBox.question(self, i18n.tr('common.confirm'), i18n.tr('save.confirm_restore').format(name=meta.get('name', ''), slots=slots_str), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply != QMessageBox.StandardButton.Yes:
            return
        try:
    ...
    def _do_delete(self):
        if not self._current_backup_meta:
            return
        meta = self._current_backup_meta
        reply = QMessageBox.question(self, i18n.tr('common.confirm'), i18n.tr('save.confirm_delete').format(name=meta.get('name', '')), QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply != QMessageBox.StandardButton.Yes:
            return
        backup_dir = _effective_backup_dir()
        try:
            save_manager.delete_backup(backup_dir, meta['id'])
            self.status_message.emit(i18n.tr('save.status_deleted').format(name=meta.get('name', '')))
            self._current_backup_meta = None
            self._refresh()
        except Exception as exc:
            QMessageBox.critical(self, i18n.tr('common.error'), str(exc))
    ...
    def _do_save_notes(self, slot: int):
        ...
        if self._source_type == self._SOURCE_GAME:
            ...
            if not meta:
                return
            try:
                updated_meta = save_manager.update_backup_slot_note(backup_dir, meta['id'], slot, name, memo)
            except Exception as exc:
                QMessageBox.critical(self, i18n.tr('common.error'), str(exc))
                return
            ...
            for lrow in range(self._left_list.count()):
                ...
                if ldata and ldata.get('type') == 'backup' and (ldata.get('meta', {}).get('id') == meta['id']):
            ...
            for row in range(self._table.rowCount()):
                ...
                if it and it.data(Qt.ItemDataRole.UserRole) == slot:
                    ...
                    break
            self._clear_edit_refs()
            self._swap_detail_widget(self._build_backup_tabbed_widget(updated_meta, slot=slot, initial_tab=1))
