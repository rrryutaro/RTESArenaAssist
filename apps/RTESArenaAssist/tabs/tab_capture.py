...
from PySide6.QtWidgets import QCheckBox, QGraphicsScene, QGraphicsView, QHBoxLayout, QLabel, QListWidget, QListWidgetItem, QMessageBox, QPushButton, QSplitter, QVBoxLayout, QWidget
...
class TabCapture(QWidget):
    ...
    def _delete_all_unlocked(self) -> None:
        ...
        self.refresh()
        try:
            self.window().statusBar().showMessage(f'キャプチャを {deleted} 件削除しました（ロック {locked} 件保持）', 5000)
        except Exception:
            pass
    ...
    def _on_select(self, row: int) -> None:
        ...
        if os.path.isfile(layout_path):
            ...
            self._load_image(self._img_game, os.path.join(self._cap_dir, f'cap_{n:03d}_game.png'))
            self._load_image(self._img_viewer, os.path.join(self._cap_dir, f'cap_{n:03d}_viewer.png'))
