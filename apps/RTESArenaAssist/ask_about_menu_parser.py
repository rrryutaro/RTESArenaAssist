...
_CC_TITLE  = (0x09, 0x60)
_CC_HOTKEY = (0x09, 0xC0)
_CC_NORMAL = (0x09, 0xD4)
_CC_NEWLINE = (0x0D, 0x00)
...
def _extract_hotkey(data: bytes) -> tuple[str, str]:
    ...
    while i < len(data):
        if i + 1 < len(data):
            ...
            if pair == _CC_HOTKEY:
                i += 2
                if i < len(data) and data[i] not in (0x00, 0x0D):
                    hotkey = chr(data[i])
...
def _scan_items_in_range(raw: bytes, start: int, end: int) -> tuple[list[str], list[str]]:
    ...
    while pos < end - 1:
        ...
        if pair in _ITEM_STARTERS:
            line_end = raw.find(b"\x0d\x00", pos, end)
            if line_end == -1:
                break
...
_OPT_LEGACY_IDS = (
    ...
    "ask_about_menu.opt_where_is.0",
    "ask_about_menu.opt_rumors.0",
...
_CHROME_SURFACE_LEGACY_IDS = {
    ...
    "Where is...": "ask_about_menu.opt_where_is.0",
    "Rumors": "ask_about_menu.opt_rumors.0",
    "Exit": "ask_about_menu.opt_exit.0",
    "Rumor Type": "ask_about_menu.title_rumor_type.0",
    "General": "ask_about_menu.opt_general.0",
    "Work": "ask_about_menu.opt_work.0",
    "Inn": "ask_about_menu.place_inn.0",
    "Temple": "ask_about_menu.place_temple.0",
    "Equipment Store": "ask_about_menu.place_equipment_store.0",
    "Mages Guild": "ask_about_menu.place_mages_guild.0",
    "Palace": "ask_about_menu.place_palace.0",
    "City gates": "ask_about_menu.place_city_gates.0",
    "Nearest Inn": "ask_about_menu.place_nearest_inn.0",
    "Nearest Temple": "ask_about_menu.place_nearest_temple.0",
    "Nearest Store": "ask_about_menu.place_nearest_store.0",
    "Nearest Dungeon": "ask_about_menu.place_nearest_dungeon.0",
}
