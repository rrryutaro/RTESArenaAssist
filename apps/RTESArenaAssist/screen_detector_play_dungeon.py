...
def detect_dungeon_play_screen(analyzer, anchor: int, img_name: str, menu_active_was_zero: bool=False, popup_foreground: bool=False) -> Tuple[str, str]:
    img_upper = (img_name or '').upper()
    menu_active = _read_u16_le(analyzer, anchor + MENU_ACTIVE_OFFSET)
    if img_upper == 'OP.IMG' and menu_active == 0 and menu_active_was_zero and (not popup_foreground):
