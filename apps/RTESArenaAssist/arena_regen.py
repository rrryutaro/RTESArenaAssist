...
_KEY_RE = re.compile('^#([0-9]+)([a-zA-Z]?)$')
_PLACEHOLDER_RE = re.compile('%([a-zA-Z][a-zA-Z0-9]*)')
...
def parse_template_dat_bytes(raw: bytes) -> list[dict]:
    ...
    while i < len(lines):
        ...
        entries.append({'key': key, 'letter': letter, 'copy': copy_idx, 'values': values})
        i = j
    return entries
...
def regenerate_building_entry_bytes(raw: bytes) -> dict[str, dict]:
    ...
    for e in entries:
        ...
        for vi, value in enumerate(e['values']):
            ...
            out[app_id] = {'original': value, 'source_id': sa.template_id(block, vi, copy=copy), 'source_hash': sa.source_hash(value), 'key': e['key'], 'letter': e['letter'] or None, 'copy': copy, 'placeholders': _placeholders(value)}
    return out
...
def build_original_json(new_entries: dict[str, dict]) -> dict:
    ...
    for app_id in sorted(new_entries):
        ...
        out[app_id] = {'original': e['original'], 'source_id': e['source_id'], 'source_hash': e['source_hash'], 'key': e['key'], 'letter': e['letter'], 'copy': e['copy'], 'placeholders': e['placeholders']}
    return out
...
def _npc_emit(out: dict[str, dict], block: str, values: list[str]) -> None:
    for vi, value in enumerate(values):
        ...
        out[app_id] = {'original': value, 'source_id': sa.template_id(block, vi), 'source_hash': sa.source_hash(value), 'placeholders': sorted(set(_placeholders(value)))}
...
def build_npc_dialog_original_json(new_entries: dict[str, dict]) -> dict:
    ...
    for app_id in sorted(new_entries):
        e = new_entries[app_id]
        out[app_id] = {'original': e['original'], 'source_id': e['source_id'], 'source_hash': e['source_hash'], 'placeholders': e['placeholders']}
    return out

def fingerprint_bytes(raw: bytes) -> str:
    return hashlib.sha256(raw).hexdigest()[:16]

def _build_manifest(new_entries: dict[str, dict], fingerprint: str, category: str, generator_version: str) -> dict:
    entries = {e['source_id']: e['source_hash'] for e in new_entries.values()}
    return {sa.MANIFEST_VERSION: generator_version, sa.MANIFEST_GENERATOR: f'arena_regen/{generator_version}', sa.MANIFEST_FINGERPRINT: fingerprint, sa.MANIFEST_DIGEST: sa.manifest_digest(entries), 'category': category, sa.MANIFEST_ENTRIES: entries}
...
def build_npc_dialog_manifest(new_entries: dict[str, dict], fingerprint: str) -> dict:
    return _build_manifest(new_entries, fingerprint, NPC_DIALOG_CATEGORY, NPC_DIALOG_GENERATOR_VERSION)
...
def _parse_inf_text_section(raw: bytes, inf_name: str) -> list[dict]:
    ...
    def new_cur() -> dict:
        return {'inf': inf_name, 'idx': None, 'type': None, 'key_id': None, 'text_lines': [], 'params': None, 'riddle_lines': [], 'answers': [], 'correct_lines': [], 'wrong_lines': [], 'riddle_mode': None}
    out: list[dict] = []
    cur = new_cur()
    in_text = False
    ...
    def flush() -> None:
        ...
        if t == 'key':
            if cur['text_lines']:
                e.update(type='key_lore', key_id=cur['key_id'], text='\n'.join(cur['text_lines']))
            else:
                e.update(type='key', key_id=cur['key_id'])
        elif t == 'lore_once':
            e.update(type='lore_once', text='\n'.join(cur['text_lines']))
        elif t == 'riddle':
            e.update(type='riddle', params=cur['params'], question='\n'.join(cur['riddle_lines']), answers=cur['answers'], correct='\n'.join(cur['correct_lines']), wrong='\n'.join(cur['wrong_lines']))
        else:
...
def _inf_entry(original: str, source_id: str, extra: dict) -> dict:
    e = {'original': original, 'source_id': source_id, 'source_hash': sa.source_hash(original)}
    e.update(extra)
    return e
    ...
    for inf_name in sorted(inf_files):
        ...
        for pe in _parse_inf_text_section(inf_files[inf_name], inf_name.upper()):
            ...
            if t in ('lore', 'lore_once'):
                out[base] = _inf_entry(pe['text'], sid, {**common, 'text': pe['text']})
            elif t == 'key_lore':
                out[base] = _inf_entry(pe['text'], sid, {**common, 'key_id': pe['key_id'], 'text': pe['text']})
            elif t == 'key':
                out[base] = _inf_entry('', sid, {**common, 'key_id': pe['key_id']})
            elif t == 'riddle':
                out[base] = _inf_entry('', sid, {**common, 'params': pe['params'], 'question': pe['question'], 'answers': pe['answers'], 'correct': pe['correct'], 'wrong': pe['wrong']})
                for field in ('question', 'correct', 'wrong'):
                    text = pe[field]
                    if not text:
                        continue
                    out[f'{INF_TEXT_CATEGORY}.{inf}_{idx}.{field}'] = _inf_entry(text, f'{sid}:{field}', {'type': 'riddle', 'field': field})
    return out
...
def build_inf_text_manifest(new_entries: dict[str, dict], fingerprint: str) -> dict:
    return _build_manifest(new_entries, fingerprint, INF_TEXT_CATEGORY, INF_TEXT_GENERATOR_VERSION)
CHARGEN_QUESTION_GENERATOR_VERSION = 'question-2'
_CHARGEN_QUESTION_NUMBER_RE = re.compile('^\\s*\\d+\\.\\s*')
_CHARGEN_CATEGORY_MARKER_RE = re.compile('\\s*\\(5[lcv]\\)\\s*')
...
def regenerate_chargen_questions(raw: bytes) -> dict[str, dict]:
    ...
    for i, (desc, a, b, c) in enumerate(_parse_question_txt(raw), start=1):
        ...
        out[base] = _inf_entry(text, sid, {'inf': inf, 'idx': 0, 'type': 'lore', 'text': text, 'text_display': full, 'text_panel': full})
        out[f'{INF_TEXT_CATEGORY}.{inf}_0.display'] = _inf_entry(full, f'{sid}:display', {'type': 'lore', 'field': 'display'})
    return out

def build_chargen_questions_manifest(new_entries: dict[str, dict], fingerprint: str) -> dict:
    return _build_manifest(new_entries, fingerprint, INF_TEXT_CATEGORY, CHARGEN_QUESTION_GENERATOR_VERSION)
...
def regenerate_atrade_tavern(raw: bytes) -> dict[str, dict]:
    ...
    for i, s in enumerate(_read_dat_strings(raw)):
        ...
        out[app_id] = {'original': text, 'source_id': sa.tradetext_id('tavern', i), 'source_hash': sa.source_hash(text), 'placeholders': _placeholders(text)}
    return out
...
def regenerate_atrade_shops(equip_raw: bytes, selling_raw: bytes, muguild_raw: bytes) -> dict[str, dict]:
    dats = {'equip': _read_dat_strings(equip_raw), 'selling': _read_dat_strings(selling_raw), 'muguild': _read_dat_strings(muguild_raw)}
    out: dict[str, dict] = {}
    ...
    def emit(app_id: str, datkey: str, idx: int) -> None:
        ...
        out[f'{NPC_DIALOG_KEY_PREFIX}{app_id}'] = {'original': text, 'source_id': sa.tradetext_id(datkey, idx), 'source_hash': sa.source_hash(text), 'placeholders': _placeholders(text)}
...
def build_atrade_shop_manifest(new_entries: dict[str, dict], fingerprint: str) -> dict:
    return _build_manifest(new_entries, fingerprint, NPC_DIALOG_CATEGORY, ATRADE_SHOP_GENERATOR_VERSION)
AKEY_REPAIR_GENERATOR_VERSION = 'akeyrepair-1'
_AKEY_AMT_PH = re.compile('%(?:t|a|mm)\\b')
_AKEY_REPAIR_MAP = {'A182.0': ('1424', 1, ()), 'A183.0': ('1424', 0, ()), 'A184.0': ('1425', 0, ()), 'A185.0': ('1426', 0, (('gp', 'gold'),)), 'A185.1': ('1426', 0, ()), 'A186.0': ('1427', 0, ()), 'A186.1': ('1427', 1, ()), 'A187.0': ('1428', 0, ()), 'A187.1': ('1428', 1, ()), 'A188.0': ('1417', 0, ()), 'A188.1': ('1418', 0, (('cost, but', 'cost but'),)), 'A188.2': ('1418', 0, ())}
...
def regenerate_akey_repair(template_raw: bytes) -> dict[str, dict]:
    ...
    for akey, (block, vi, subs) in _AKEY_REPAIR_MAP.items():
        ...
        out[f'{NPC_DIALOG_KEY_PREFIX}{akey}'] = {'original': text, 'source_id': sa.template_id(block, vi, copy=0), 'source_hash': sa.source_hash(text), 'placeholders': _placeholders(text)}
    return out

def build_akey_repair_manifest(new_entries: dict[str, dict], fingerprint: str) -> dict:
    return _build_manifest(new_entries, fingerprint, NPC_DIALOG_CATEGORY, AKEY_REPAIR_GENERATOR_VERSION)
...
_AKEY_EXE_PH = re.compile('%l?[sduSDU]\\d*')
...
def regenerate_akey_ui(raw_map: dict, template: dict) -> dict[str, dict]:
    ...
    for akey, spec in template.items():
        ...
        if mode == 'seg':
            ...
            joined = re.sub('\\s+', ' ', raw.replace('\r', ' ')).strip()
        ...
        out[app_id] = {'original': text, 'source_id': sa.aexe_id('akey', akey), 'source_hash': sa.source_hash(text), 'placeholders': _placeholders(text)}
    return out

def build_akey_ui_manifest(new_entries: dict[str, dict], fingerprint: str) -> dict:
    return _build_manifest(new_entries, fingerprint, NPC_DIALOG_CATEGORY, AKEY_UI_GENERATOR_VERSION)
CHARGEN_UI_GENERATOR_VERSION = 'chargenui-1'
_CHARGEN_CLASS_ORDER = ('MAGE', 'SPELLSWORD', 'BATTLEMAGE', 'SORCEROR', 'HEALER', 'NIGHTBLADE', 'BARD', 'BURGLAR', 'ROGUE', 'ACROBAT', 'THIEF', 'ASSASSIN', 'MONK', 'ARCHER', 'RANGER', 'BARBARIAN', 'WARRIOR', 'KNIGHT')
_CHARGEN_RACE_ORDER = ('BRETON', 'REDGUARD', 'NORD', 'DARK_ELF', 'HIGH_ELF', 'WOOD_ELF', 'KHAJIIT', 'ARGONIAN')
_CHARGEN_UI_SINGLE = {'': 'choose_class_creation', '10Q': 'class_questions_intro', 'CHOOSE_CLASS': 'choose_class_list', 'GENDER': 'choose_gender', 'CHOOSE_ATTRIBUTES': 'choose_attributes', 'BONUS_REMAINING': 'choose_attributes_bonus_points_remaining', 'APPEARANCE': 'choose_appearance', 'GOYENOW': 'confirmed_race4'}

def _chargen_norm(s: str) -> str:
    return re.sub('\\s+', ' ', s.replace('\r', ' ')).strip()
...
def _chargen_ui_emit(out: dict, name: str, text: str, src_id: str) -> None:
    ...
    base = f'{INF_TEXT_CATEGORY}.{inf}_0.0'
    out[base] = _inf_entry(text, src_id, {'inf': inf, 'idx': 0, 'type': 'lore', 'text': text, 'text_display': text, 'text_panel': text})
    out[f'{INF_TEXT_CATEGORY}.{inf}_0.display'] = _inf_entry(text, f'{src_id}:display', {'type': 'lore', 'field': 'display'})

def regenerate_chargen_ui(cc: dict, class_names: list, pref_attrs: list, race_descs: list) -> dict[str, dict]:
    out: dict[str, dict] = {}
    for name, key in _CHARGEN_UI_SINGLE.items():
        raw = cc.get(key)
        if not raw:
            continue
        _chargen_ui_emit(out, name, _chargen_norm(raw), _chargen_ui_src('ui', key))
    ...
    if sc:
        ...
        for i, cls in enumerate(_CHARGEN_CLASS_ORDER):
            ...
            _chargen_ui_emit(out, f'RESULT_{cls}', text, _chargen_ui_src('result', i))
    ...
    if cr3:
        ...
        for i, cls in enumerate(_CHARGEN_CLASS_ORDER):
            ...
            _chargen_ui_emit(out, f'CLASS_ADVICE_{cls}', text, _chargen_ui_src('advice', i))
    ...
    if cr2:
        ...
        for i, race in enumerate(_CHARGEN_RACE_ORDER):
            ...
            _chargen_ui_emit(out, f'RACE_{race}', text, _chargen_ui_src('race', i))
    return out
...
def regenerate_npc_name_chunks_bytes(raw: bytes) -> dict[str, dict]:
    ...
    while off + 3 <= n:
        ...
        for si in range(string_count):
            ...
            out[app_id] = {'original': value, 'source_id': sa.namechnk_id(chunk_idx, si), 'source_hash': sa.source_hash(value)}
        off += chunk_length
        chunk_idx += 1
    return out
...
def build_npc_name_chunks_manifest(new_entries: dict[str, dict], fingerprint: str) -> dict:
    return _build_manifest(new_entries, fingerprint, NPC_NAME_CHUNKS_CATEGORY, NPC_NAME_CHUNKS_GENERATOR_VERSION)
