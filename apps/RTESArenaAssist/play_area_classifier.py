...
def detect_play_area(analyzer, anchor: Optional[int], mif_name: Optional[str]) -> PlayArea:
...
def resolve_in_interior(interior_flag: Optional[int], place_byte: Optional[int], mif_name: Optional[str]) -> bool:
    raw = interior_flag is not None and interior_flag != 0
    if not raw:
        return False
    if place_byte in (0, 1) and classify_play_area(mif_name) != 'dungeon':
        return False
    return True
