...
def resolve_attach_path(w, *, mif_name: str='', in_interior: bool=False, facility_kind: str='') -> Dict[str, Any]:
    l1 = current_state(w)
    if l1 != 'normal-play':
        return {'l1': l1, 'l2': '', 'l3': '', 'facility': None}
    l2 = classify_base_location(getattr(w, '_analyzer', None), getattr(w, '_anchor', None), mif_name)
    if in_interior:
