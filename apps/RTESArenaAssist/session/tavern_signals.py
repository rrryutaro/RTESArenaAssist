...
def gather_tavern_signals(analyzer, anchor: int, *, shop_kind: str, shop_owner: str, img: str, in_interior: bool, facility_tavern: bool, npc_phase: Optional[int]=None) -> TavernSignals:
    img_u = (img or '').upper()
    ...
    try:
        ...
        _raw, _canon, _rendered, _text = read_negotiation_diagnostic(analyzer, anchor)
        if _text:
    ...
    try:
        from popup11_response_reader import read_response_candidates_all, read_current_text_pointer as _rcp, candidate_contains_pointer
        _rc = read_response_candidates_all(analyzer, anchor)
        _ptr = _safe(lambda: _rcp(analyzer, anchor), None)
        npc_response_hit = any((c.text and c.lookup_hit and candidate_contains_pointer(c, _ptr) for c in _rc))
    except Exception:
        npc_response_hit = False
    rumor_marker = False
    try:
        ...
        if npc_phase == NPC_PHASE_ASKING:
            ...
            if _mk:
                ...
                rumor_marker = detect_active_sub_menu_title(_parsed, _mk) == 'Rumor Type'
    except Exception:
        rumor_marker = False
    return TavernSignals(in_interior=bool(in_interior), facility_tavern=bool(facility_tavern), shop_kind=shop_kind or 'none', shop_owner=shop_owner or '', img=img_u, active_surfaces=frozenset(active_surfaces), cur_ptr_surface=cur_ptr_surface, negotiation_body=negotiation_body, negotiation_prompts=counter_active, counter_active=counter_active, npc_response_hit=npc_response_hit, rumor_marker=rumor_marker)
