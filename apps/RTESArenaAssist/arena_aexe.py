...
def _detect_image_base_by_scan(analyzer) -> Optional[tuple[str, int]]:
    ...
    return None
THIEVING_DIVISORS_OFFSETS = {'A.EXE': 254266, 'ACD.EXE': 255038}
THIEVING_DIVISORS_COUNT = 18

def read_thieving_divisors(analyzer) -> Optional[list[int]]:
    ...
    if off is None:
        return None
    try:
        raw = analyzer.read_bytes(image_base + off, THIEVING_DIVISORS_COUNT)
    except OSError as e:
