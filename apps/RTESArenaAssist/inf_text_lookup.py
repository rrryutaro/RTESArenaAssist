...
def _load_v2() -> dict[tuple[str, int], dict]:
    ...
    for e in i18n.v2_category_entries(_CATEGORY):
        sid = e.get('source_id')
        rich = e.get('rich')
        if isinstance(rich, dict) and rich.get('inf') is not None and (rich.get('idx') is not None):
            inf = rich.get('inf')
            ridx = rich.get('idx')
            entry = dict(rich)
        else:
...
def get_translation(entry: dict, lang: str='ja') -> str | dict | None:
    ...
    if entry.get('_v2'):
        ...
        if entry.get('type') == 'riddle':
            return {fld: i18n.text_by_source_id(f'{sid}:{fld}', category=_CATEGORY) or '' for fld in ('question', 'correct', 'wrong')}
        return i18n.text_by_source_id(sid, category=_CATEGORY) or ''
    base = _base_of(entry.get('_id', ''))
    if entry.get('type') == 'riddle':
        return {fld: i18n.lang_only(f'{base}.{fld}') or '' for fld in ('question', 'correct', 'wrong')}
...
def get_text_panel(entry: dict) -> str:
    if entry is None:
        return ''
    return entry.get('text_panel') or entry.get('text_display') or entry.get('text', '')

def get_text_display(entry: dict) -> str:
    if entry is None:
        return ''
    return entry.get('text_display') or entry.get('text_panel') or entry.get('text', '')
