...
class TempleResponseCandidate(NamedTuple):
    ...
    source_offset: int
    raw_text: str = ''
_FMT_PREFIX_RE = re.compile('^([0-9]{3})([A-Za-z].*)$', re.S)
_HEAL_OFFER_CONTAM_RE = re.compile('Can I give you some of\\s+(.+?(?:is in perfect condition|thou art healed).*)$', re.S)
_HEALED_RESULT_RE = re.compile('^(?P<subject>.+?)(?P<suffix>,\\s*thou art healed.*)$', re.S)
_PERFECT_RESULT_RE = re.compile('^(?P<subject>.+?)(?P<suffix>\\s+is in perfect condition.*)$', re.S)
...
_RESPONSE_REGIONS: tuple[tuple[int, int], ...] = ((37534, 512), (4160, 512))
_MIN_RESPONSE_LEN = 5
_CURRENT_TEXT_PTR_OFFSET = 43076
...
_PHASE_AUX_OFFSET = 43079
_PHASE_MODE_OFFSET = 43085
_PHASE_SELECT_OFFSET = 43067
_PHASE_ACTIVE_VALUE = 117
_RESULT_VIEW_PTR_OFFSET = 36718
_RESULT_INTENT_HINT_OFFSET = 44470
_BLESS_RESULT_INTENT_VALUE = 119
_BLESS_RESULT_VIEW_LO_BYTES = frozenset({89, 90})
_LEGACY_BLESS_RESULT_PTR = 7514
...
_RESULT_EDGE_OFFSETS = (36732, 36734, 36754, 36756)
...
def read_temple_result_edge_signature(analyzer, anchor: int) -> tuple[int, ...] | None:
    try:
        vals = []
...
def read_temple_response_candidates(analyzer, anchor: int) -> TempleResponseRead:
    try:
        import npc_dialog_lookup as ndl
    except ImportError:
        ndl = None
    ...
    for off, raw_text, prev_b in _scan_runs(analyzer, anchor):
        ...
        if not hit:
            continue
        candidates.append(TempleResponseCandidate(text=canon, lookup_hit=True, source_offset=off, raw_text=raw_text))
    try:
