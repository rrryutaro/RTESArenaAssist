...
def _prune_history(history_dir: str) -> None:
    if _HISTORY_KEEP <= 0:
        return
    try:
        files = sorted((f for f in os.listdir(history_dir) if f.startswith('assist_') and f.endswith('.log')))
    except OSError:
        return
...
def init(app_dir: str) -> None:
    ...
    try:
        ...
        history_path = os.path.join(history_dir, f'assist_{stamp}_{os.getpid()}.log')
        history_fh = logging.FileHandler(history_path, encoding='utf-8', mode='w')
    ...
    try:
        ...
        _ver = 'unknown'
    logging.warning('=== RTESArenaAssist %s started (log: %s) ===', _ver, log_path)
