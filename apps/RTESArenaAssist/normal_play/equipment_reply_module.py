...
_INITIAL_HIT_IMGS = frozenset({'YESNO.IMG'})
_DIALOG_HIT_IMGS = frozenset({'YESNO.IMG', 'FACES00.CIF', 'STATUS.CIF'})
...
_NEWPOP_PROMPT_PREFIXES = ('I can cut down the time', 'I can cut the cost')
_NEWPOP_RESULT_PREFIXES = ("Then I'll get started", "Good, I'll get to it", 'I understand. You might consider', 'Well, if you change your mind')
...
def _read_active_reply_candidates(analyzer, anchor: int, ndl) -> list['ResponseCandidate']:
    try:
...
def _is_terminal_repair_reply_text(text: str) -> bool:
    ...
    if text.startswith('Your '):
        return 'does not need any repairing' in text or text.endswith(' is ready.')
...
def _is_equipment_repair_reply_text(text: str) -> bool:
    ...
    if text.startswith('Your ') and 'repair' not in text:
        return False
    return text.startswith(('Your ', 'Fixing that ', 'Sure I could fix that ', 'Fine. I can get it done in ', "Fine, I'll charge you ", "Then I'll get started", "Good, I'll get to it", 'I understand. You might consider', 'Well, if you change your mind', "Can't you afford it?", "Can't you wait that long?", "Maybe you're not interested?", 'Which job do you wish to inspect?', 'Sorry, I already have my hands full.', 'I can cut down the time', 'I can cut the cost'))
...
def _menu_rt_equipment_menu_present(w) -> bool:
    try:
        ...
        from shop_menu_reader import parse_menu_groups, select_menu_group_by_ptr, SHOP_MENU_BUFFER_OFFSET, SHOP_MENU_BUFFER_MAXLEN
...
def poll_equipment_reply(w, *, equipment_active: bool, equipment_just_started: bool, img_name: str, shop_menu_visible: bool) -> bool:
    ...
    if equipment_just_started:
        _reset_state(w)
    try:
        from popup11_response_reader import ResponseCandidate, candidate_contains_pointer, read_current_text_pointer, read_response_candidates_all
        import npc_dialog_lookup as _ndl
        candidates = read_response_candidates_all(w._analyzer, w._anchor)
        active_reply_candidates = _read_active_reply_candidates(w._analyzer, w._anchor, _ndl)
    ...
    hits = [c for c in hits if not (_is_terminal_repair_reply_text(c.text) and _is_terminal_reply_suppressed(w, c.text))]
    current_reply_text = getattr(w, '_equipment_reply_current_text', '') or ''
    has_unsuppressed_repair_hit = any((_is_equipment_repair_reply_text(c.text) for c in hits))
    if _is_terminal_reply_suppressed(w, current_reply_text):
        w._equipment_reply_current_key = None
        w._equipment_reply_current_text = None
        w._equipment_reply_hold_polls = 0
        hold_polls = 0
    newpop_prompt_hits = [c for c in hits if img == 'NEWPOP.IMG' and (c.text or '').startswith(_NEWPOP_PROMPT_PREFIXES)]
    newpop_result_hits = [c for c in hits if img == 'NEWPOP.IMG' and (c.text or '').startswith(_NEWPOP_RESULT_PREFIXES)]
    newpop_no_repair_hits = [c for c in hits if img == 'NEWPOP.IMG' and _is_no_repair_reply_text(c.text)]
    ...
    if img == 'MENU_RT.IMG' and (shop_menu_visible or _menu_rt_equipment_menu_present(w)) and (not (getattr(w, '_equipment_reply_polled_in_render', False) and (has_unsuppressed_repair_hit or _is_terminal_repair_reply_text(current_reply_text)))):
        w._equipment_reply_hold_polls = 0
        w._equipment_reply_current_key = None
        w._equipment_reply_current_text = None
        return False
    if shop_menu_visible and getattr(w, '_equipment_menu_return_override', False):
        w._equipment_reply_hold_polls = 0
        w._equipment_reply_current_key = None
        w._equipment_reply_current_text = None
    ...
    if ptr_hits:
        ...
        chosen = hits[0]
        reason = 'dialog_hit'
    elif hits and (not baselined) and (img in _INITIAL_HIT_IMGS) and (not shop_menu_visible):
        chosen = hits[0]
        ...
        if hold_polls > 0 and current_text:
            for c in candidates:
                ...
                source_offset = current_key[1] if isinstance(current_key, tuple) and len(current_key) >= 2 and isinstance(current_key[1], int) else 0
                chosen = ResponseCandidate(text=current_text, lookup_hit=True, source_offset=source_offset)
                reason = 'hold_stale'
        else:
            chosen = None
            reason = ''
    if chosen is None:
        w._equipment_reply_hold_polls = 0
        if _is_no_repair_reply_text(getattr(w, '_equipment_reply_current_text', '') or ''):
            w._equipment_reply_current_key = None
            w._equipment_reply_current_text = None
        return False
    ...
    choice_lines = _read_active_reply_choice_group(w._analyzer, w._anchor, chosen.source_offset)
    if choice_lines:
    ...
    key = (img, chosen.source_offset, chosen.text, ja_text, tuple(((row['en'], row['ja']) for row in choice_rows)))
    ...
    if reason in ('source_changed', 'dialog_hit', 'newpop_no_repair'):
        ...
        w._equipment_reply_hold_polls = 0
    should_update = key != w._equipment_reply_current_key or (owner_taken and reason in ('ptr', 'source_changed', 'active_reply', 'dialog_hit', 'newpop_prompt', 'newpop_result', 'hold', 'hold_stale', 'initial_dialog_hit'))
    if should_update:
        w._equipment_reply_current_key = key
        w._equipment_reply_current_text = chosen.text
        if choice_rows:
            w._ui_router.update_facility_list(REPLY_OWNER, choice_rows, 'Repair Options', '修理の選択肢')
        else:
            w._ui_router.update_translation(REPLY_OWNER, en_text, ja_text, speech_role='conversation', speech_text=ja)
        _log.info('equipment_reply translated: src=0x%X reason=%s img=%r text=%r', chosen.source_offset, reason, img, chosen.text[:80])
    return True
__all__ = ['poll_equipment_reply', 'reset_equipment_reply_state', 'REPLY_OWNER']
