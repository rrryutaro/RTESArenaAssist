...
def _dialog_body_source(ptr: int | None) -> tuple[int, int, bool] | None:
    ...
    if message_buffer_remaining(ptr) is not None:
        return (ptr, _DIALOG_BUF_OFFSET + _DIALOG_BUF_READ - ptr, False)
...
def _note_body_fit(fits: bool, off: int, size: int) -> None:
    if _fit_state.get('ok', True) is fits:
        return
    _fit_state['ok'] = fits
    if not fits:
...
def _read_dialog_chunks(w, off: int, size: int) -> list[str]:
    try:
        ...
        return []
    chunks = _dialog_chunks(raw)
    if chunks:
        _note_body_fit(True, off, size)
    elif b'\x00' not in raw and _is_text_bytes(raw):
        _note_body_fit(False, off, size)
    return chunks
