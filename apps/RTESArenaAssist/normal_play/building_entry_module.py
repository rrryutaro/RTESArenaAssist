...
def should_poll_building_entry(*, entry_phase: bool, panel_owner: str, pending: bool, img_name: str) -> bool:
    ...
    if img_upper in _NON_ENTRY_XMI_IMGS:
        return False
    entry_xmi_surface = img_upper.endswith('.XMI') and img_upper not in _DIALOG_MENU_IMGS
...
def _diag_dump_memory(w, msg_buf: str, npc_dialog: str) -> None:
    ...
    _log.warning('entry diag [%d/%d]: msg_buf=%r npc_dialog=%r', _dump_count + 1, _MAX_DIAG_DUMPS, msg_buf[:120] if msg_buf else '', npc_dialog[:120] if npc_dialog else '')
    _diag_offsets = ((4164, 512, '0x1044 npc_dialog'), (37534, 512, '0x929E b131 buffer'), (39582, 512, '0x9A9E msg_buf'), (39034, 512, '0x987A negot_rendered'), (36864, 512, '0x9000+512'), (37888, 512, '0x9400+512'), (38400, 512, '0x9600+512'), (38912, 512, '0x9800+512'))
        ...
        try:
            ...
            while _pos < len(_raw) and len(_chunks) < 6:
                if _raw[_pos] == 0:
                    ...
                    continue
                _end = _pos
                while _end < len(_raw) and _raw[_end] != 0:
                    _end += 1
                _frag = _raw[_pos:_end].decode('ascii', errors='replace')
                _printable = sum((1 for c in _frag if 32 <= ord(c) <= 126))
                if _frag and _printable / len(_frag) >= 0.8 and (len(_frag.strip()) >= 4):
                    _chunks.append(_frag.strip()[:120])
                _pos = _end + 1
            if _chunks:
                _log.warning('entry diag [%d/%d] %s: %s', _dump_count + 1, _MAX_DIAG_DUMPS, _label, ' | '.join((f'{c!r}' for c in _chunks[:4])))
        except (OSError, AttributeError) as _exc:
            _log.debug('entry diag %s read failed: %s', _label, _exc)

def _read_b131_buffer(w) -> str:
    try:
        _raw = w._analyzer.read_bytes(w._anchor + 37534, 256)
...
def _read_b131_chunks(w) -> list[str]:
    try:
        raw = w._analyzer.read_bytes(w._anchor + 37534, 512)
    except (OSError, AttributeError):
        return []
    chunks: list[str] = []
    pos = 0
...
def _read_msg_buf_chunks(w) -> list[str]:
    try:
        raw = w._analyzer.read_bytes(w._anchor + 39582, 512)
    except (OSError, AttributeError):
        return []
    chunks: list[str] = []
...
def poll_building_entry(w, *, building_entry_active: bool, entry_phase_prev: bool, msg_buf: str, npc_dialog: str) -> bool:
    ...
    if building_entry_active:
        try:
            ...
            _entry_candidates = [*[(f'msg_buf_full[{idx}]', txt) for idx, txt in enumerate(_full_texts)], ('msg_buf', msg_buf), ('b131', _read_b131_buffer(w)), ('npc_dialog', npc_dialog)]
