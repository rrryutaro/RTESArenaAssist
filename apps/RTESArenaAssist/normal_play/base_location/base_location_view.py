...
def resolve_field_facility_entry(hint: Optional[FieldEntranceContext], *, interior_flag_nonzero: bool, wild_flag: int) -> Tuple[bool, Optional[str], str]:
...
_INTERIOR_MIF_PREFIXES: Tuple[str, ...] = ('TAVERN', 'TEMPLE', 'EQUIP', 'ARMORS', 'MAGES', 'MAGE', 'PALACE', 'NOBLE', 'HOUSE')

def _looks_like_interior_mif(mif_name: Optional[str]) -> bool:
    u = (mif_name or '').upper()
    return any((u.startswith(p) for p in _INTERIOR_MIF_PREFIXES))

def classify_base_location(analyzer, anchor: Optional[int], mif_name: Optional[str]) -> str:
...
def resolve_area_with_indoor_fallback(analyzer, anchor: Optional[int], mif_name: Optional[str], in_interior: bool, last_non_interior_area: str) -> Tuple[str, str]:
    ...
    if area in ('city', 'wilderness', 'dungeon'):
        return (area, area)
    return (area, last_non_interior_area)
__all__ = ['FieldEntranceContext', 'resolve_field_facility_entry', 'classify_base_location', 'area_name', 'classify_map_axis', 'resolve_area_with_indoor_fallback']
