...
_PLAYERDATA_WILD_OFF = 1536
_PLAYERDATA_WILD_LEN = 1562 - 1536
...
def _wild_menu_texture_sets() -> tuple[frozenset[int], frozenset[int]]:
    ...
    extended = game | frozenset((menu_map[m] for m in _WILD_EXTENDED_MENU_IDS if m in menu_map))
    _wild_menu_tex_sets = (game, extended)
    return _wild_menu_tex_sets
_C3_ENTRY_CANDIDATES = [(43266, 'a902'), (43264, 'a900'), (43268, 'a904'), (43272, 'a908'), (43136, 'a880'), (43086, 'a84e'), (43088, 'a850'), (43090, 'a852'), (43096, 'a858')]
...
class WildernessMapSession(MapSessionBase):
    ...
    def update(self, ctx: MapContext) -> None:
        ...
        if wild_seed == 0:
            _log.warning("wild: location_name='%s' is too short for wildSeed", ctx.location_name)
            return
        if ctx.player_tile_x is not None and ctx.player_tile_y is not None:
            ...
            if ws is not None:
                ...
                self._player_x = rt_x
                self._player_y = rt_z
                if (self._chunk_tracker.chunk_x, self._chunk_tracker.chunk_y) != (bx, by):
                ...
                if self._seed_pending:
                    self._seed_pending = False
                    same_field = self._last_wild_seed is not None and self._last_wild_seed == wild_seed
                    near = self._last_rt is not None and abs(rt_x - self._last_rt[0]) <= _ROUNDTRIP_TOL and (abs(rt_z - self._last_rt[1]) <= _ROUNDTRIP_TOL)
                    if not (same_field and near):
                        seed_x = CITY_ORIGIN_CHUNK_X if rt_x >= _HALF_CHUNK else CITY_ORIGIN_CHUNK_X - 1
                        seed_y = CITY_ORIGIN_CHUNK_Y if rt_z >= _HALF_CHUNK else CITY_ORIGIN_CHUNK_Y - 1
                        self._chunk_tracker.reset(seed_x, seed_y)
                self._chunk_tracker.update(rt_x, rt_z)
                self._center_origin = (self._chunk_tracker.chunk_x, self._chunk_tracker.chunk_y)
            self._last_rt = (rt_x, rt_z)
            origin, n_chunks, live_origin, live_blocks = self._compute_display_plan(rt_x, rt_z)
            ...
            live_origin = None
            live_blocks = None
        if not (wild_seed != self._wild_seed or origin != self._origin_chunk or n_chunks != self._n_chunks or (live_origin != self._built_live_origin) or (live_blocks != self._built_live_blocks) or (self._walkable is None)):
            return
        blocks = get_block_lists(ctx.analyzer)
        try:
            grid = build_wild_voxel_grid(wild_seed=wild_seed, blocks=blocks, player_voxel_x=0, player_voxel_y=0, origin_chunk=origin, flip_x=False, n_chunks=n_chunks, live_origin_chunk=live_origin, live_wild_blocks=live_blocks)
        except Exception:
            _log.exception('build_wild_voxel_grid failed (loc=%s seed=0x%08X)', ctx.location_name, wild_seed)
            return
    ...
    def _read_wild_state(self, ctx: MapContext, wild_seed: int) -> Optional[tuple[int, int, int, int, tuple[int, ...]]]:
        if ctx.analyzer is None or ctx.anchor is None:
            return None
        try:
            raw = ctx.analyzer.read_bytes(ctx.anchor + _PLAYERDATA_WILD_OFF, _PLAYERDATA_WILD_LEN)
        except Exception:
            return None
    ...
    def _compute_origin(self, rt_x: int, rt_z: int) -> tuple[int, int]:
        o = self._center_origin or (self._chunk_tracker.chunk_x, self._chunk_tracker.chunk_y)
        actual_cx = o[0] + (1 if rt_x >= _CHUNK else 0)
        actual_cy = o[1] + (1 if rt_z >= _CHUNK else 0)
        return (actual_cx - 1, actual_cy - 1)

    def _compute_display_plan(self, rt_x: int, rt_z: int) -> tuple[tuple[int, int], int, Optional[tuple[int, int]], Optional[tuple[int, ...]]]:
        ...
        gx, gy = centered
        fully_outside = gx + _N_CHUNKS - 1 < 0 or gx > WILD_WIDTH - 1 or gy + _N_CHUNKS - 1 < 0 or (gy > WILD_HEIGHT - 1)
        if fully_outside:
    ...
    def _wild_entrance_cells(self) -> tuple[tuple[int, int], ...]:
        ...
        if key != self._entrance_key:
            ...
            self._entrance_cells = tuple(detect_menu_cells(self._map1, set(tex_set)))
    ...
    def _wild_edge_marks(self) -> tuple[tuple[int, int, str], ...]:
        if self._map1 is None or not self._show_edge or (not self._distinguish_edge):
            return ()
        key = id(self._map1)
        if key != self._edge_marks_key:
            from services.wild_edges import get_wild_edge_category_map, extract_edge_marks
    ...
    def _wild_crop_marks(self) -> tuple[tuple[int, int, str], ...]:
        ...
        if key != self._crop_marks_key:
            from services.wild_edges import get_wild_crop_category_map, extract_crop_marks
    ...
    def _enterable_menu_cells(self) -> tuple[tuple[int, int, int], ...]:
        ...
        if key != self._enterable_key:
            ...
            enter_tex = {tex: mid for tex, mid in tex2menu.items() if mid in _WILD_ENTERABLE_MENU_IDS}
    ...
    def _resolve_field_door_mif(self, abs_x: int, abs_y: int, menu_id: int) -> Optional[str]:
        try:
            ...
            return get_door_voxel_mif_name(abs_x, abs_y, menu_id, 0, False, ArenaCityType.CITY_STATE, MapType.WILDERNESS)
        except Exception:
            _log.exception('field door mif resolve failed')
            return None

    def _update_field_entrance_hint(self, local_x: Optional[int], local_y: Optional[int]) -> None:
    ...
    def _resolve_field_facility_name(self, abs_x: int, abs_y: int, menu_id: int) -> tuple[str, Optional[str]]:
        ...
        try:
            if menu_id == 3:
                from services.building_name_generator import generate_wild_tavern_name_opentes, make_wild_chunk_name_seed
            ...
            if menu_id == 4:
                ...
                from services.building_name_generator import generate_wild_temple_name_calibrated, make_wild_temple_name_seed_calibrated
    ...
    def _log_field_entrance_calibration(self, abs_x: int, abs_y: int, cx: int, cz: int, menu_id: int, mif: Optional[str]) -> None:
        from services.arena_level_utils import get_door_voxel_mif_name
        from services.arena_voxel_utils import MapType
        from services.arena_types import ArenaCityType
        cand = {}
        for name, (x, y) in {'noswap(abs_x,abs_y)=OTA採用': (abs_x, abs_y), 'swap(abs_y,abs_x)': (abs_y, abs_x)}.items():
            try:
                cand[name] = get_door_voxel_mif_name(x, y, menu_id, 0, False, ArenaCityType.CITY_STATE, MapType.WILDERNESS)
            except Exception:
                cand[name] = '<err>'
        _log.warning('FIELD_ENTRANCE menu=%s(id=%d) cell=(%d,%d) abs=(%d,%d) mif=%s || cand: %s', _MENU_LABELS.get(menu_id, str(menu_id)), menu_id, cx, cz, abs_x, abs_y, mif, ' / '.join((f'{k}={v}' for k, v in cand.items())))
    ...
    def _log_c3_entry_coords(self, ctx: MapContext, rt_x: int, rt_z: int, origin: tuple[int, int]) -> None:
        ...
        _log.warning('C3_ENTRY loc=%s rt=(%d,%d) hi=(%d,%d) center_origin=(%d,%d) actual_chunk=(%d,%d) voxel=(%d,%d) grid_origin=(%d,%d) marker=(%d,%d) || %s', ctx.location_name, rt_x, rt_z, rt_x >> 8 & 255, rt_z >> 8 & 255, o_x, o_y, ac_x, ac_y, rt_x % _CHUNK, rt_z % _CHUNK, origin[0], origin[1], mk_x, mk_y, cand)

    def get_canvas_data(self) -> CanvasData:
        local_x: Optional[int] = None
        local_y: Optional[int] = None
        if self._player_x is not None and self._player_y is not None and (self._origin_chunk is not None):
            rt_x = int(self._player_x)
            rt_z = int(self._player_y)
            o = self._center_origin or (self._chunk_tracker.chunk_x, self._chunk_tracker.chunk_y)
            if 0 <= rt_x <= _RT_MAX_DISPLAY and 0 <= rt_z <= _RT_MAX_DISPLAY:
