...
_KEY_PREV = '_equipment_negot_key_prev'
_DIAG_KEY_PREV = '_equipment_negot_diag_key_prev'
_PROMPTS_CTX_PREV = '_equipment_negot_prompts_ctx_prev'
_EMPTY_POLLS = '_equipment_negot_empty_polls'
_COUNTER_ACTIVE = '_equipment_negot_counter_active'
_SPEECH_PREV = '_equipment_negot_speech_prev'
...
def reset_equipment_negotiation_state(w) -> None:
    setattr(w, _KEY_PREV, None)
    setattr(w, _DIAG_KEY_PREV, None)
    setattr(w, _PROMPTS_CTX_PREV, None)
    setattr(w, _EMPTY_POLLS, 0)
...
def poll_equipment_negotiation(w, *, img_name: str, top_level_state: str) -> bool:
    _ensure_state(w)
    setattr(w, _COUNTER_ACTIVE, False)
    ...
    try:
        ...
        _log.exception('equipment negotiation_reader failed')
    ...
    if getattr(w, _DIAG_KEY_PREV) != _diag_key:
        ...
        _log.info('equipment negotiation template raw=%r canonical=%r rendered=%r matched=%r suffix=%r', (_raw or '')[:80], (_canon or '')[:80], (_rendered or '')[:80], (_text or '')[:80], _suffix)
    try:
    ...
    if _text and _ndl is not None:
        try:
            ...
            _log.exception('equipment negotiation lookup failed')
            _r = None
    _fallback_body = ''
    if _text and _r is None:
    ...
    if _ndl is not None:
        try:
            ...
            _ap_ctx_key = (img_name, top_level_state, 'equipment_negot')
            ...
            for c in read_active_template_candidates(w._analyzer, w._anchor):
                ...
                if not _ap_clean:
                    continue
                if _same_sentence(_ap_clean, _text):
                    continue
            ...
            _log.exception('equipment negotiation prompts read failed')
    ...
    if getattr(w, _EMPTY_POLLS) >= _EMPTY_POLLS_THRESHOLD:
        _log.info('equipment negotiation exit: empty body+prompts for %d polls (img=%r)', getattr(w, _EMPTY_POLLS), img_name)
        return False
    ...
    if _key != getattr(w, _KEY_PREV):
        ...
        _log.info('equipment negotiation translated: body=%r prompts=%d', (_text or '')[:80], len(_active_prompts_pairs))
    return True

def cleanup_equipment_negotiation_if_owner(w) -> None:
    _ensure_state(w)
    try:
        if w._ui_router.is_owner(NEGOTIATION_OWNER):
            w._ui_router.clear_if_owner(NEGOTIATION_OWNER)
            _log.info('equipment negotiation exit (cleanup)')
    except AttributeError:
        pass
    reset_equipment_negotiation_state(w)
__all__ = ['NEGOTIATION_OWNER', 'poll_equipment_negotiation', 'cleanup_equipment_negotiation_if_owner', 'reset_equipment_negotiation_state', 'compute_speech_diff']
