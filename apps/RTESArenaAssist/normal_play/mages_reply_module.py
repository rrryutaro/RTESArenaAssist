...
def _read_detect_magic_already_known(w, current_ptr, candidates, *, force: bool=False) -> ResponseCandidate | None:
    if not force:
        if not (isinstance(current_ptr, int) and _MAGES_MENU_PTR_START <= current_ptr < _MAGES_MENU_PTR_END):
            return None
        if not any((_DETECT_MAGIC_QUOTE_PREFIX in _normalize_reply_text(c.text) for c in candidates)):
            return None
    try:
        raw = w._analyzer.read_bytes(w._anchor + _MAGES_MENU_TEXT_OFFSET, 80)
...
def _detect_magic_result_candidate(w, candidates) -> ResponseCandidate | None:
    for c in candidates:
        if _DETECT_MAGIC_IDENTIFIED in _normalize_reply_text(c.text):
            return ResponseCandidate(text=_DETECT_MAGIC_IDENTIFIED, lookup_hit=True, source_offset=c.source_offset)
    try:
...
def poll_mages_reply(w, *, mages_active: bool, mages_just_started: bool, img_name: str, shop_menu_visible: bool) -> bool:
    ...
    def _norm_hit(text: str) -> bool:
        try:
            ...
            return False
    hits = [c for c in candidates if c.text and (c.lookup_hit or _norm_hit(c.text))]
    detect_known = _read_detect_magic_already_known(w, current_ptr, candidates, force=True) if detect_kind == 'detect_known' else None
    detect_cost = _detect_magic_cost_candidate(candidates) if detect_kind == 'detect_cost' else None
    detect_result = _detect_magic_result_candidate(w, candidates) if detect_kind == 'detect_result' else None
    ...
    if should_update:
        ...
        _log.info('mages_reply translated: src=0x%X reason=%s img=%r text=%r', chosen.source_offset, reason, img, chosen.text[:80])
    return True
__all__ = ['poll_mages_reply', 'reset_mages_reply_state', 'REPLY_OWNER']
