...
def _stabilize_list_items(w, img: str, items: list[dict]) -> list[dict]:
    ...
    if len(items) < len(stable):
        ...
        setattr(w, _LIST_PENDING_ATTR, pending_by_img)
        if count < 3:
            _log.info('equipment_list transient partial suppressed (img=%r stable=%d candidate=%d count=%d)', img, len(stable), len(items), count)
            return [dict(it) for it in stable]
    stable_by_img[img] = [dict(it) for it in items]
    pending_by_img.pop(img, None)
    setattr(w, _LIST_STABLE_ATTR, stable_by_img)
...
def _list_signature(items: list[dict]) -> tuple:
    return tuple(((it.get('en', ''), it.get('hands', ''), it.get('protects', ''), it.get('protects_ja', ''), it.get('weight', ''), it.get('price_display', '')) for it in items))
...
def _load_static_weapon_items() -> list[dict]:
    ...
    for id_, entry in orig.items():
        ...
        out.append({'en': en, 'ja': ja, 'hands': _format_static_hands(data.get('handed')), 'weight': _format_weight(data.get('weight')), 'price_raw': price_text, 'price_display': price_text})
...
__all__ = ['_read_list_items', '_stabilize_list_items', '_load_static_weapon_items']
