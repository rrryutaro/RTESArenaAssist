...
_DIALOG_OFF_MIN = 36864
_DIALOG_OFF_MAX = 40960
...
def poll_palace_dialog(w, *, palace_active: bool) -> bool:
    ...
    def _emit(off: int, text: str, ja: str, *, yesno: bool) -> bool:
        ...
        if getattr(w, '_palace_dialog_prev_key', None) == display_key and w._ui_router.is_owner(_OWNER):
            return True
        w._palace_dialog_prev_key = display_key
        w._ui_router.update_translation(_OWNER, text, ja, speech_role='conversation')
        _log.info('panel_owner -> palace_dialog (off=0x%04X yesno=%s text=%r)', off, yesno, text[:60])
        return True
    if in_band_fg:
