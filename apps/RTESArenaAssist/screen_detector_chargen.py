...
def detect_chargen_screen(chargen_hint: Optional[str], img_name: str, last_subscreen: Optional[str]=None) -> Optional[Tuple[str, str]]:
    ...
    if img_upper.startswith('INTRO') and img_upper.endswith('.IMG'):
        try:
            ...
            pass
    if chargen_hint == 'opening_cinematic' or (last_subscreen == 'opening_cinematic' and img_upper.startswith('FACES') and img_upper.endswith('.CIF')):
    ...
    if img_upper == 'SCROLL02.DFA':
        sub = last_subscreen if last_subscreen in ('ten_questions', 'class_select') else 'ten_questions'
        return (sub, _tr(sub))
