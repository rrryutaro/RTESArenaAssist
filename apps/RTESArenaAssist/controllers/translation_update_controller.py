...
def update_translate_tab(win, entry: dict) -> None:
    ...
    if inf_key == '_CHARGEN_':
        ...
        win._chargen._reset_chargen_state_for_restart(reason='method NPC detected (new chargen start)')
        ...
        if saved_streak >= 2:
            ...
            _log.info('chargen: method state captured at NPC detection = 0x%02X (streak=%d)', saved_state_prev, saved_streak)
        try:
    ...
    if typ == 'riddle':
        ...
        win._push_translation(tab_orig, tab_trans, panel_original=panel_orig, panel_translated=panel_trans)

def push_translation(win, original: str, translated: str, panel_original: str | None=None, panel_translated: str | None=None, speech_role: str | None=None) -> None:
    chargen_sub = None
    try:
        ...
        pass
    if not original and (not translated) and (chargen_sub is not None) and (getattr(win, '_last_chargen_subscreen', None) == chargen_sub):
        _log.debug('push_translation: chargen cache hit (skip empty push, sub=%s)', chargen_sub)
        return
    try:
        ...
        if win._chargen_class_list_active or mode == 'class_list':
            ...
            mode = 'translate'
        if mode == 'load_screen':
            img_name_now = (getattr(win, '_img_name_prev', '') or '').upper()
    ...
    win._ui_router.update_translation('', original, translated, mode=None, panel_en=p_orig, panel_ja=p_trans, keep_owner=True, speech_role=speech_role, speech_text=_speech_text)
