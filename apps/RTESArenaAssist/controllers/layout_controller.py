...
class LayoutController:
    ...
    def open_layout_settings_menu(self, pos=None):
        ...
        _CORNER_DATA = [(LayoutCorner.TOP_LEFT, 'layout.corner_tl', self.on_corner_tl), (LayoutCorner.TOP_RIGHT, 'layout.corner_tr', self.on_corner_tr), (LayoutCorner.BOTTOM_LEFT, 'layout.corner_bl', self.on_corner_bl), (LayoutCorner.BOTTOM_RIGHT, 'layout.corner_br', self.on_corner_br)]
        ...
        _FORM_DATA = [(LayoutForm.FORM_1, 'layout.form_1'), (LayoutForm.FORM_2, 'layout.form_2'), (LayoutForm.FORM_3, 'layout.form_3')]
    ...
    def toggle_dosbox_topmost(self, checked: bool):
        ...
        w._sb.showMessage('DOSBox 最前面表示: ON' if checked else 'DOSBox 最前面表示: OFF', 3000)
    ...
    def arrange_layout(self):
        ...
        if not ok:
            QMessageBox.warning(w, i18n.tr('common.warning'), i18n.tr('layout.dosbox_not_found'))
            return
        w._sb.showMessage(f'配置完了: DOSBox {dos_w}×{dos_h}  [{w._layout_corner.value}]', 5000)
    ...
    def enter_layout_mode(self):
        ...
        _log.info('pre-layout: logical_w=%d  phys_w=%d  screen_dpr=%.3f  meas_dpr=%.4f', _pre_log_w, _pre_phys_w, screen_dpr, meas_dpr)
        ...
        _log.info('layout(log): %dx%d  dos=(%d,%d)%dx%d  panel=(%d,0)%dx%d', lw_log, lh_log, dos_x_log, dos_y_log, dos_w_log, dos_h_log, panel_x_log, panel_w_log, lh_log)
        ...
        _log.info('assist phys: (%d,%d) %dx%d  actual_dpr=%.4f', assist_phys_x, assist_phys_y, _phys_w, _assist_phys.bottom - _assist_phys.top, actual_dpr)
        ...
        zones = calc_layout_zones(w._layout_form, corner, dos_w_log, dos_h_log, lw_log, lh_log)
        ...
        if not ok:
            ...
            QMessageBox.warning(w, i18n.tr('common.warning'), 'DOSBox の配置に失敗しました。\nDOSBox が起動中か確認して再試行してください。')
            return
        _dos_hwnd = w._layout_mgr.get_dosbox_hwnd()
        _assist_hwnd = int(w.winId())
        ctypes.windll.user32.ShowWindow(_dos_hwnd, 4)
        ctypes.windll.user32.SetWindowPos(_dos_hwnd, _assist_hwnd, 0, 0, 0, 0, 19)
        ctypes.windll.user32.BringWindowToTop(_assist_hwnd)
        w._layout_dos_offset = (dos_x_log, dos_y_log)
    ...
    def exit_layout_mode(self):
        ...
        if _dos_hwnd:
            ...
            if w._layout_dosbox_saved_rect is not None:
                ...
                ret = ctypes.windll.user32.SetWindowPos(_dos_hwnd, None, r[0], r[1], r[2] - r[0], r[3] - r[1], 16)
                _log.info('restored dosbox pos: (%d,%d) %dx%d ret=%s', r[0], r[1], r[2] - r[0], r[3] - r[1], ret)
        w._layout_dosbox_saved_rect = None
    ...
    def enter_embed_layout_mode(self):
        ...
        if output_mode.lower() != 'surface':
            QMessageBox.warning(w, '埋め込みレイアウトモード', f"DOSBox の出力モードが surface ではありません（現在: {output_mode or '不明'}）。\n\n埋め込みモードは surface モードのみ対応しています。\nDOSBox タブで output を surface に変更して DOSBox を再起動してください。")
            return
        corner = w._layout_corner
        screen = (w.screen() or QApplication.primaryScreen()).geometry()
    ...
    def handle_move_event(self):
        ...
        dos_w, dos_h = w._layout_dos_size
