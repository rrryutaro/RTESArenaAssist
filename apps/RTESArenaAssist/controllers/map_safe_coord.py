...
DIALOG_IMGS_FOR_MAP: frozenset = frozenset({'YESNO.IMG', 'NEGOTBUT.IMG', 'NEWPOP.IMG', 'POPUP11.IMG', 'FACES00.CIF'})
OWNER_FACILITY_OR_NEGOT: frozenset = frozenset({'tavern_yesno', 'tavern_rumor_type', 'tavern_negotiation', 'negotiation', 'shop_menu', 'shop_rumor_type', 'shop_buy', 'shop_rooms'})
...
def resolve_post_exit_coord(*, guard_active: bool, guard_remaining: int, stale_rt: Optional[tuple], door: Optional[tuple], raw_xy: tuple, show_xy: tuple):
    ...
    return (False, new_remaining, show_xy[0], show_xy[1], False)
...
def compute_map_safe_coord(*, img_name: str, npc_phase: Optional[int], is_building_entry_msg: bool, facility_active: bool, owner: str, raw_x: Optional[int], raw_y: Optional[int], raw_angle: Optional[float], last_x: Optional[int], last_y: Optional[int], last_angle: Optional[float], npc_phase_idle_value: int=_NPC_PHASE_IDLE) -> MapSafeCoord:
    ...
    if unsafe_reasons:
        last_pair = (last_x, last_y) if last_x is not None and last_y is not None else None
        if last_pair is not None and last_pair not in INVALID_HELD_COORDS:
            return MapSafeCoord(source='held', player_x=last_x, player_y=last_y, angle_deg=last_angle, unsafe_reasons=unsafe_reasons)
        return MapSafeCoord(source='none', player_x=None, player_y=None, angle_deg=None, unsafe_reasons=unsafe_reasons)
    return MapSafeCoord(source='raw', player_x=raw_x, player_y=raw_y, angle_deg=raw_angle, unsafe_reasons=[])
__all__ = ['MapSafeCoord', 'compute_map_safe_coord', 'resolve_post_exit_coord', 'DIALOG_IMGS_FOR_MAP', 'INVALID_HELD_COORDS', 'OWNER_FACILITY_OR_NEGOT', 'POST_EXIT_GUARD_MAX_POLLS']
