...
def _is_interactive(widget) -> bool:
    ...
    if _INTERACTIVE_TYPES is None:
        from PySide6.QtWidgets import QAbstractButton, QAbstractScrollArea, QAbstractSlider, QLineEdit, QComboBox, QTabBar, QAbstractItemView, QSplitterHandle
        _INTERACTIVE_TYPES = (QAbstractButton, QAbstractScrollArea, QAbstractSlider, QLineEdit, QComboBox, QTabBar, QAbstractItemView, QSplitterHandle)
...
class WindowChrome:
    ...
    def __init__(self, window):
        ...
        self._edge_cursors = {'n': QCursor(Qt.CursorShape.SizeVerCursor), 's': QCursor(Qt.CursorShape.SizeVerCursor), 'e': QCursor(Qt.CursorShape.SizeHorCursor), 'w': QCursor(Qt.CursorShape.SizeHorCursor), 'nw': QCursor(Qt.CursorShape.SizeFDiagCursor), 'se': QCursor(Qt.CursorShape.SizeFDiagCursor), 'ne': QCursor(Qt.CursorShape.SizeBDiagCursor), 'sw': QCursor(Qt.CursorShape.SizeBDiagCursor)}
    ...
    def do_resize(self, gpos: QPoint):
        ...
        if geo.width() < WIN_MIN_W:
            if 'w' in d:
                geo.setLeft(geo.right() - WIN_MIN_W)
            else:
                geo.setRight(geo.left() + WIN_MIN_W)
        if geo.height() < WIN_MIN_H:
            if 'n' in d:
                geo.setTop(geo.bottom() - WIN_MIN_H)
            else:
                geo.setBottom(geo.top() + WIN_MIN_H)
    ...
    def handle_event(self, obj, event) -> bool:
        ...
        if obj is not w and (not w.isAncestorOf(obj)):
            return False
        et = event.type()
        gpos = event.globalPosition().toPoint() if hasattr(event, 'globalPosition') else None
        if self._resize_dir:
        ...
        if et == QEvent.Type.MouseMove and event.buttons() == Qt.MouseButton.LeftButton and (self._drag_start_pos is not None) and (not self._resize_dir) and gpos:
