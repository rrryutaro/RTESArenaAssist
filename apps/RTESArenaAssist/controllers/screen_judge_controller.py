...
class ScreenJudgeController:
    ...
    def capture_now(self) -> Optional[Image.Image]:
        if not self._aspect_warned:
            try:
                ...
                if aspect_str == 'true':
                    _log.warning('DOSBox aspect=true detected — coord_mapper does not compensate letterbox; observation points may be off')
            except Exception:
                pass
    ...
    def detect_npc_dialog(self, purpose_filter: Optional[str]=None, threshold: float=0.75, capture_first: bool=False) -> DetectionResult:
        if capture_first:
            self.capture_now()
        img = self._last_capture
        mapper = self._last_mapper
        if img is None or mapper is None:
            return DetectionResult(state=DialogState.UNKNOWN, hit_count=0, total_count=0, hit_ratio=0.0, detail='no capture available')
        result = detect_dialog(img=img, mapper=mapper, registry=self._registry, purpose_filter=purpose_filter, threshold=threshold)
