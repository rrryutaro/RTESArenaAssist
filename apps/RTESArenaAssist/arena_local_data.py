...
_AEXE_CONTENT_VERSION = ''
...
def _loose_names_upper(arena_dir: str) -> set[str]:
    try:
        return {n.upper() for n in os.listdir(arena_dir) if os.path.isfile(os.path.join(arena_dir, n))}
    except OSError:
        return set()
...
def detect_running_arena_dir() -> str | None:
    ...
    bases = [os.path.dirname(dosbox_dir), dosbox_dir, os.path.dirname(os.path.dirname(dosbox_dir))]
    for base in bases:
...
def _arena_asset_hashes(arena_dir: str) -> dict:
    out: dict[str, str] = {}
    try:
        real = {n.upper(): n for n in os.listdir(arena_dir) if os.path.isfile(os.path.join(arena_dir, n))}
    except OSError:
        return out
...
def _load_fingerprint_manifest() -> dict:
    ...
    if txt is not None:
        try:
            ...
            pass
    logger.warning('arena_local_data: fingerprint manifest missing/invalid: %s', _FINGERPRINT_MANIFEST_PATH)
    return {'asset_sets': []}

def classify_arena_dir(arena_dir: str, hashes: dict | None=None) -> dict:
    base = {'status': 'invalid', 'set_id': None, 'label': None, 'exe_kind': None, 'hashes': {}, 'supports_aexe_offsets': False, 'supports_akey_acd_offsets': False}
    ...
    for s in _load_fingerprint_manifest().get('asset_sets', []):
        ...
        if files and all((h.get(k) == v for k, v in files.items())):
            return {'status': 'verified', 'set_id': s.get('id'), 'label': s.get('label'), 'exe_kind': s.get('exe_kind'), 'hashes': h, 'supports_aexe_offsets': bool(s.get('supports_aexe_offsets')), 'supports_akey_acd_offsets': bool(s.get('supports_akey_acd_offsets'))}
    return {'status': 'unknown', 'set_id': None, 'label': None, 'exe_kind': None, 'hashes': h, 'supports_aexe_offsets': False, 'supports_akey_acd_offsets': False}
...
def _harvest_aexe(analyzer, supports: bool=True, progress=None, scan_progress=None, cancel_check=None):
...
def _harvest_akey_ui(analyzer, supports: bool=True) -> dict:
    ...
    if txt is None:
        logger.warning('arena_local_data: akey template missing/invalid: %s', _AKEY_TEMPLATE_PATH)
        return {}
    try:
...
def _build_chargen_ui(tables: dict, template_raw: bytes) -> dict:
    import arena_regen
    cc = {k.split('.', 1)[1]: v for k, v in tables.items() if k.startswith('char_creation.')}
...
def verify_against_golden(local_manifests: dict[str, dict], golden: dict, local_exe_harvest: bool) -> dict:
    ...
    for cat, gman in golden_cats.items():
        ...
        per_cat[cat] = {'missing': res['counts']['missing'], 'drift': res['counts']['drift'], 'extra': res['counts']['extra'], 'missing_ids': res['missing'][:20], 'drift_ids': res['drift'][:20]}
        total_missing += counted_missing
        total_drift += res['counts']['drift']
    ok = total_missing == 0 and total_drift == 0
    return {'ok': ok, 'categories': per_cat, 'missing': total_missing, 'drift': total_drift, 'summary': f'missing={total_missing},drift={total_drift}', 'soften_missing': soften_missing}

def _run_golden_check(local_manifests: dict[str, dict], asset_set: str | None, exe_harvest: bool) -> str:
    ...
    if gres['ok']:
        try:
            import assist_log
            assist_log.recog(logger, 'arena_local_data: ゴールデンマニフェスト突合せ一致 (%d カテゴリ・%s)', len(gres['categories']), golden.get('asset_set') or '-')
        except Exception:
            pass
        return 'ok'
...
def build_local_pack(arena_dir: str, user_dir: str, analyzer=None, classification: dict | None=None, progress=None, cancel_check=None) -> str | None:
    ...
    if cls.get('status') != 'verified':
        logger.warning('arena_local_data: 未検証の Arena データです（status=%s・対応指紋セット不一致）。一部 EXE 由来文字列（A.EXE テーブル / A-key UI）は生成されません。DAT/TEMPLATE/VFS 由来の再生成は継続します。', cls.get('status'))
    ...
    inft = arena_regen.regenerate_inf_text_bytes(_read_inf_files(arena_dir, progress=lambda done, total: _p(0.12 + 0.1 * (done / max(1, total)), 'INF テキストを読込中…')))
    from arena_vfs import Vfs
    question_raw = Vfs(arena_dir).read('QUESTION.TXT')
    chargen_q = arena_regen.regenerate_chargen_questions(question_raw) if question_raw is not None else {}
    inft.update(chargen_q)
    tavern_raw = Vfs(arena_dir).read('TAVERN.DAT')
    atrade = arena_regen.regenerate_atrade_tavern(tavern_raw) if tavern_raw is not None else {}
    npcd.update(atrade)
    _vfs = Vfs(arena_dir)
    eq, se, mu = (_vfs.read('EQUIP.DAT'), _vfs.read('SELLING.DAT'), _vfs.read('MUGUILD.DAT'))
    atrade_shop = arena_regen.regenerate_atrade_shops(eq, se, mu) if eq and se and mu else {}
    ...
    nnc = arena_regen.regenerate_npc_name_chunks_bytes(namechnk_raw) if namechnk_raw is not None else {}
    _ck()
    _p(0.3, 'メモリからデータを採取中…')
    aexe = _harvest_aexe(analyzer, exe_ok, cancel_check=cancel_check, scan_progress=lambda done, total: _p(0.3 + 0.2 * (done / max(1, total)), 'メモリからデータを採取中…'), progress=lambda done, total: _p(0.5 + 0.12 * (done / max(1, total)), 'メモリからデータを採取中…'))
    ...
    if analyzer is not None and exe_ok:
        ...
        try:
            ...
            if cg:
                city_gen_json = {'source': 'ACD.EXE (Steam・memory harvest)', 'data': cg[1]}
        except Exception as e:
    ...
    akey_complete = not akey_ok or bool(akey_ui)
    content_version = _AEXE_CONTENT_VERSION if aexe_cats and akey_complete else _BASE_CONTENT_VERSION
    if akey_ok and aexe_cats and (not akey_ui):
        logger.warning('arena_local_data: A-key UI 採取が空のため AEXE content_version を刻みません（次回も再生成対象）')
    ...
    local_manifests = {arena_regen.CATEGORY: m_be, arena_regen.NPC_DIALOG_CATEGORY: m_npcd, arena_regen.INF_TEXT_CATEGORY: m_inft}
    if m_nnc:
        local_manifests[arena_regen.NPC_NAME_CHUNKS_CATEGORY] = m_nnc
    aexe_manifests = {cat: arena_regen.build_aexe_manifest(cat, oj, fp) for cat, oj in aexe_cats.items()}
    ...
    if city_gen_json is not None:
        from services import citydata_reader
        m_city_gen = citydata_reader.build_city_generation_manifest(city_gen_json['data'], fp)
    ...
    logger.info('arena_local_data: built local data (fp=%s, building_entry=%d, npc_dialog=%d[atrade=%d,atradeshop=%d,akeyrepair=%d,akeyui=%d], inf_text=%d[chargen_q=%d,chargen_ui=%d], npc_name_chunks=%d, aexe=%s[%s], golden=%s)', fp, len(be), len(npcd), len(atrade), len(atrade_shop), len(akey_repair), len(akey_ui), len(inft), len(chargen_q), len(chargen_ui), len(nnc), len(aexe_cats), aexe_version or '-', golden_check)
...
def _read_spellsg65(arena_dir: str) -> bytes | None:
    ...
    if raw:
        return raw
    for cand in (os.path.join(arena_dir, 'save', 'SPELLSG.65'), os.path.join(arena_dir, 'save', 'Spellsg.65')):
...
def _write_v2_localpack(out_path: str, surface_by_source_id: dict[str, str], fp: str, rich_by_source_id: dict[str, dict] | None=None, generated_assets: dict[str, bytes] | None=None, content_version: str | None=None, asset_set_id: str | None=None, asset_hashes: str | None=None, exe_harvested: bool | None=None) -> None:
    try:
        ...
        if os.path.exists(tmp_out):
            os.remove(tmp_out)
        summary = localpack_builder.write_localpack_by_source_id(tmp_out, smap, surface_by_source_id, registry_hash=registry_hash, spell_effect_entries=spell_effect_entries, rich_by_source_id=rich_by_source_id, arena_fingerprint=fp, generated_assets=generated_assets, provider='arena_local_data')
        try:
            from arena_pack import ArenaPack
            import i18n_localpack as _ilp
            with ArenaPack.open(tmp_out) as _lpk:
                _lpk.set_meta(_ilp.META_REGISTRY_VERSION, str(int(bundle.get('registry_version', 0) or 0)))
...
def v2_localpack_update_status(user_dir: str) -> dict | None:
    ...
    try:
        ...
        return {'needed': True, 'axis': 'unreadable', 'from': 0, 'to': _V2_BUILDER_VERSION, 'is_dev': _is_dev_build()}
    bundle = _load_owned_bundle()
    if bundle is None:
        return None
    if lp.builder_version < _V2_BUILDER_VERSION:
        return {'needed': True, 'axis': 'builder', 'from': lp.builder_version, 'to': _V2_BUILDER_VERSION, 'is_dev': _is_dev_build()}
    if _is_dev_build():
        ...
        if lp.registry_version < bv:
            return {'needed': True, 'axis': 'registry', 'from': lp.registry_version, 'to': bv, 'is_dev': False}
    return None
...
def v2_localpack_needs_regen(arena_dir: str, user_dir: str, analyzer_available: bool, classification: dict | None=None) -> bool:
    try:
        raw, fp = _current_template_fingerprint(arena_dir)
        if raw is None:
            return False
        ...
        return True
__all__ = ['v2_localpack_path', 'is_valid_arena_dir', 'detect_arena_dirs', 'detect_running_arena_dir', 'classify_arena_dir', 'build_local_pack', 'v2_localpack_needs_regen', 'rebuild_v2_localpack_standalone', 'v2_localpack_update_status', 'load_golden_manifest', 'verify_against_golden']
