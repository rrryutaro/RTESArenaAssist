...
ROOM_NAMES_OFFSET = 10386
ROOM_PRICES_OFFSET = 10435
ROOM_COUNT = 5
_EXPECTED_NAMES = ('Single', 'Double', 'Suite', "King's Suite", "Emperor's Suite")
_NAME_MIN = 32
_NAME_MAX = 126
...
def read_room_list(analyzer: 'ArenaMemoryAnalyzer', anchor: int) -> list[dict]:
    try:
        raw_names = analyzer.read_bytes(anchor + ROOM_NAMES_OFFSET, 64)
        raw_prices = analyzer.read_bytes(anchor + ROOM_PRICES_OFFSET, ROOM_COUNT * 2)
    ...
    for name, price in zip(names, prices):
        items.append({'en': name, 'price_raw': str(price), 'price_display': str(price)})
    return items

def translate_room_list(items: list[dict], *, section: str='rooms') -> list[dict]:
    out: list[dict] = []
    for it in items:
        en = it.get('en', '')
    ...
    return out
__all__ = ['ROOM_NAMES_OFFSET', 'ROOM_PRICES_OFFSET', 'ROOM_COUNT', 'parse_room_names', 'parse_room_prices', 'read_room_list', 'translate_room_list']
