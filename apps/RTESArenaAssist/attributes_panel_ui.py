...
def build_main_grid(panel: 'AttributesPanel') -> QWidget:
    ...
    for row, lbl in ((ROW_NAME, panel._name_lbl), (ROW_RACE, panel._race_lbl), (ROW_CLASS, panel._class_lbl)):
        lbl.setObjectName('valueLabel')
        g.addWidget(lbl, row, 0, 1, 4, alignment=Qt.AlignmentFlag.AlignLeft)
    ...
    for idx in range(8):
        ...
        if d_key:
            ...
            g.addWidget(d_value, row, COL_DERIVED_VALUE, alignment=Qt.AlignmentFlag.AlignLeft)
    ...
    g.addWidget(bp_widget, ROW_BONUS_PTS, 0, 1, 6, alignment=Qt.AlignmentFlag.AlignHCenter)
    ...
    for row, key in stat_rows:
        ...
        g.addWidget(v_lbl, row, 1, 1, 3, alignment=Qt.AlignmentFlag.AlignLeft)
...
def build_cheat_values_group(panel: 'AttributesPanel') -> QGroupBox:
    ...
    for row, (key, label_key, vmax) in enumerate(panel._CHEAT_VALUE_SPECS):
        ...
        btn.clicked.connect(lambda _checked=False, k=key: panel._write_cheat_value(k))
