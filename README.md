...
ツールで、DOSBox のメモリを読み取って表示します。既定では読み取りのみで動作します。

ただし、利用者が明示的に有効にした場合に限り、次の書き込みを行います。

- **チート機能**（既定 OFF・有効化時に確認あり）: 能力値の変更、マップの全表示、体力・疲労・
  呪文ポイントの常時最大。実行中のゲームのメモリへ書き込みます。ゲームバランスを損ねるため、
  必要な場合だけ有効にしてください。
- **セーブデータのバックアップ / 復元**: バックアップはセーブファイルを複製します。復元を実行
  すると、対象のセーブスロットへ書き戻します。
