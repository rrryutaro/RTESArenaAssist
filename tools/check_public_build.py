...
REVIEW = 'REVIEW'
ALLOW = 'ALLOW'
ARENA_ASSET_EXTS_DENY = {'.mif', '.inf', '.rmd', '.bsa', '.cfa', '.dfa', '.cif', '.voc', '.xmi', '.ci2', '.65'}
ARENA_ASSET_EXTS_REVIEW = {'.img', '.set', '.col', '.xfm', '.dat'}
ARENA_ASSET_FILENAMES_DENY = {'template.dat', 'namechnk.dat', 'spellmkr.txt', 'citydata.65', 'global.bsa', 'a.exe', 'acd.exe'}
DENY_PATH_SEGMENTS = ('arena-data/', 'i18n/_original', 'i18n/en', 'manual/full')
DENY_EXACT = {'i18n/en.json', 'aexe_strings.json', 'i18n/i18n_id_registry.json', 'i18n/legacy_id_map.json', 'i18n/location_citydata_map.json', 'i18n/placeholder_values_oc_source.json', 'i18n/placeholder_values_derived_map.json'}
REVIEW_PATH_SEGMENTS = ('services/data/', 'manual/simple')
...
@lru_cache(maxsize=1)
def _public_i18n_language_dirs() -> frozenset[str]:
    meta_path = Path(__file__).resolve().parents[1] / 'apps' / 'RTESArenaAssist' / 'i18n' / '_meta.json'
    try:
        meta = json.loads(meta_path.read_text(encoding='utf-8'))
    except (OSError, ValueError):
        return frozenset({'ja', 'es', 'de', 'fr'})
    langs = meta.get('languages', {}) if isinstance(meta, dict) else {}
    out = set()
    if isinstance(langs, dict):
        for code, spec in langs.items():
            if code == 'en' or not isinstance(spec, dict):
                continue
            if spec.get('public_enabled') is True or spec.get('status') == 'public_enabled':
                out.add(str(code).lower())
    return frozenset(out)

def _non_public_i18n_language_dir(d: str) -> str | None:
    m = re.match('(?:.*?/)?i18n/([a-z][a-z0-9_-]*)/', d)
    if not m:
        return None
    code = m.group(1)
    if code in {'en', 'ui'} or code.startswith('_'):
        return None
    if code not in _public_i18n_language_dirs():
        return code
    return None
...
def classify_dest(dest: str) -> tuple[str, str]:
    ...
    non_public_lang = _non_public_i18n_language_dir(d)
    if non_public_lang:
        return (REVIEW, f"non-public i18n language dir '{non_public_lang}'")
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Tuple, ast.List)) and len(node.elts) == 2:
            ...
            if isinstance(second, ast.Constant) and isinstance(second.value, str):
                ...
                if base and os.path.splitext(base)[1] and ('/' not in base) and ('\\' not in base):
                    dests.add(dest_dir.rstrip('/\\') + '/' + base)
                else:
                    dests.add(dest_dir)
    MIXED_DIR_MOUNTS = {'i18n': 'i18n ツリー全体を同梱（_original/en を含む）。ja/es/ui/_meta のみ明示同梱に分割せよ'}
    norm_dests = {_norm(d).rstrip('/') for d in dests}
    for dest in sorted(dests):
        nd = _norm(dest).rstrip('/')
        if nd in MIXED_DIR_MOUNTS:
            has_explicit_subdir = any((other != nd and other.startswith(nd + '/') for other in norm_dests))
            if has_explicit_subdir:
                continue
            report.add(Finding(REVIEW, 'spec.datas', dest, MIXED_DIR_MOUNTS[nd], source=spec_path))
            continue
...
def _iter_toc_entries(toc_path: str):
    ...
    def walk(obj):
        if isinstance(obj, (list, tuple)):
            if len(obj) == 3 and isinstance(obj[0], str) and isinstance(obj[1], (str, type(None))) and isinstance(obj[2], str):
...
def check_toc(build_dir: str, report: Report) -> None:
    if not os.path.isdir(build_dir):
        ...
        return
    toc_files = [os.path.join(build_dir, n) for n in sorted(os.listdir(build_dir)) if n.lower().endswith('.toc')]
    if not toc_files:
...
_BSA_HINT_NAME = re.compile('global\\.bsa$', re.IGNORECASE)
...
def check_dist(dist_dir: str, report: Report, exe_names: tuple[str, ...]=()) -> None:
    ...
    for root, dirs, files in os.walk(dist_dir):
        ...
        for name in files:
            ...
            if sig:
                report.add(Finding(REVIEW, 'dist:signature', rel, f'content looks Arena-derived: {sig}', source=full))
...
def check_bundle_public_safe(bundle_path: str, report: Report, *, dest: str='i18n_bundle.json') -> None:
        ...
        if cat.get('source_policy') != 'assist_bundled':
            for e in cat.get('entries', []):
                ...
                if dn and _debug_name_unsafe(dn):
                    report.add(Finding(DENY, 'bundle', dest, f"non-structural debug_name (原文/旧ID 疑い) id={e.get('id')}: {dn!r}"))
    for loc in bundle.get('locales', []):
        ...
        for t in loc.get('texts', []):
            ...
            if tid in nonassist_ids:
                report.add(Finding(DENY, 'bundle', dest, f'en text on non-assist_bundled id={tid}（Arena 原文 en 漏洩）'))
        ...
        try:
            for name in os.listdir(parent):
                if name.lower() in owned and os.path.isdir(os.path.join(parent, name)):
                    rel = os.path.relpath(os.path.join(parent, name), dist_dir).replace('\\', '/')
                    report.add(Finding(DENY, 'internal-deps-only', rel, 'Assist 所有データが _internal/dist に在る（_internal は Python 依存のみ・exe 内 seed へ移すこと）'))
        except OSError:
            continue
...
def check_absolute_paths(targets: list[str], report: Report) -> None:
    ...
    for path in targets:
        ...
        for h in sorted(hits):
            report.add(Finding(REVIEW, 'abspath', os.path.basename(path), f'local user path embedded: {h}'))
...
def render_json(report: Report) -> str:
    return json.dumps({'checked': report.checked, 'notes': report.notes, 'deny': [vars(f) for f in report.denies], 'review': [vars(f) for f in report.reviews], 'pass': len(report.denies) == 0}, ensure_ascii=False, indent=2)
...
def main(argv: list[str] | None=None) -> int:
    ...
    ap.add_argument('--spec', default='RTESArenaAssist-public.spec')
    ap.add_argument('--build-dir', default=os.path.join('build', 'RTESArenaAssist-public'))
    ap.add_argument('--dist', default=os.path.join('dist', 'RTESArenaAssist-public'))
    ap.add_argument('--json', action='store_true', help='JSON 出力')
    ap.add_argument('--strict-review', action='store_true', help='REVIEW も失敗扱いにする')
